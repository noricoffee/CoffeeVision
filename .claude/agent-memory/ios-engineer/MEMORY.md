...
- [AppKit アイコン生成 script パターン](appkit-icon-generation.md) — `generate_app_icon.swift`（NSBezierPath 自前描画、SF Symbols 禁止）、透過 PNG の穴あけは `.copy` 合成、`eyeOutline` は正方形前提なのでオフスクリーン `NSImage` 合成で回避
