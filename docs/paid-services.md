...
- **アプリ外の一時コスト**: おすすめカフェのシード生成 `scripts/seed/generate-curated-cafes.mjs`（フェーズ 19）が Text Search を叩く（2 クエリ × subAreas 数）。対象は **9 県**（東京 / 大阪 / 京都 / 神奈川 / 愛知 / 福岡 / 北海道 / 千葉 / 埼玉）で、フル実行 1 回あたり **124 回**（東京のみだった頃は 32 回）。実行は初回シードと定期リフレッシュ（Places 規約のキャッシュ規定 30 日）時のみで、**県を増やすとリフレッシュのたびのコストが線形に増える**
...
- アダプティブバナー広告 **2 面**: カフェ詳細 / マップ検索結果シート（いずれもインライン。後者は結果 3 件目の後）。iosApp View 層完結・iOS のみ。**記録タブ・分析タブには置かない**（requirements §11 の確定仕様）。
- **規約上の注意**: Places 由来のデータ（店名等）を広告リクエストのターゲティングシグナルに渡さないこと（Google Maps Platform Service Specific Terms）。実装・レビュー時に確認する。
- App ID / 広告ユニット ID（2 ユニット）は**本番発行済み**で、`Secrets.xcconfig` → `Info.plist` 注入（Places キーと同経路）。日常の開発・検証は Google デモ ID にフォールバックさせ、本番 ID の自己タップによる無効トラフィックを避ける。リリースビルドへの供給は CI（GitHub Secrets、未設定なら fail-fast）。
