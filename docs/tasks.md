...
| [~] | アカウントアップグレード（匿名 → Apple）の UI | 2026-06-17 着手。**Sign in with Apple のみ**採用（Google は見送り）。link で uid 不変＝データ引き継ぎ。Dispatch A（KMP: `AuthRepository` 拡張 + `AuthAccount` + `shared/feature/account` + `AccountViewModel`）→ Dispatch B（iOS: `AccountView` + `AppleSignInCoordinator` + Bridge）。設計は plan / [`implementation_note.md`](./implementation_note.md) 参照 |
| [~] | アカウント削除フロー | 2026-06-17 着手。`DeleteAccountUseCase`（Firestore 全 Visit+子 + ローカル DB）→ Swift で写真消去 → `deleteAuthUser` → 新規匿名で再 bootstrap。アップグレードと同 PR で実装 |
| [~] | サインアウト（設定画面で見送り分を回収） | 2026-06-17 着手。アップグレード済みアカウントのみ表示。サインアウト → 新規匿名で再 bootstrap |
