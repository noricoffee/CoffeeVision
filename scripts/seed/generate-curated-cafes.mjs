...
// 大手チェーンは 2 段目の「人気枠」（レビュー 500 件以上を上限の枠外で全件追加）を必ず通過するため、
// ここで名前落としするしかない（2026-07-28 の 9 県展開で 33 件混入したのを受けて拡充）。
//
// ⚠️ ラテン表記の "starbucks" は**意図的に入れていない**。カタカナの「スターバックス」で通常店舗
//    （スターバックスコーヒー 〇〇店）は落ちるが、`STARBUCKS RESERVE(R) ROASTERY TOKYO`（中目黒）は
//    焙煎所併設の目的地型フラッグシップとしておすすめに残す判断（2026-07-28 ユーザー確定）。
// ⚠️ 「珈琲館」は入れない。独立店の `表参道珈琲館` を巻き添えにするため（部分一致なので語の選定に注意）。
const EXCLUDED_NAME_KEYWORDS = [
  "シーシャ", "shisha", "hookah", "保護猫", "猫カフェ",
  "anakuma cafe", "禁断果実", "スターバックス", "むさしの森珈琲",
  // 大手チェーン（2026-07-28 追加）
  "コメダ", "ドトール", "ベローチェ", "星乃珈琲店", "エクセルシオール",
  "タリーズ", "tully", "上島珈琲店", "ルノアール", "倉式珈琲",
];
