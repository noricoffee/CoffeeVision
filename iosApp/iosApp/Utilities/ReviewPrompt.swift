...
enum ReviewPrompt {
    ...
    ///
    /// マイルストーンフラグがあるため、複数回呼ばれても即座に no-op で返る。
    ///
    /// - Important: 呼び出し元はビューのライフサイクルに紐づく構造化タスク（`.task` / `.task(id:)`）
    ///   から呼ぶこと。`Task { }` の非構造化タスクから呼ぶと、下記の 1.5 秒遅延中にユーザーが
    ///   分析タブを離れてもキャンセルされず、他タブの上にダイアログが出うる。
    @MainActor
