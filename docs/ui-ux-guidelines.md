...
| 役割 | 色 | 備考 |
|------|---------|------|
| アクセント | AccentColor **#8B5A2B**（dark: **#C08552**） | `Assets.xcassets` の AccentColor に設定済み |
...
| おすすめ（キュレーション） | `Color.orange`（システムカラー） | curatedCafePin（カップ + `star.fill` バッジ。下記「おすすめピンを色で区別しない理由」）。表示は Apple 周辺ピンと同じズームゲート（可視半径 3000m 以内）でズームイン時のみ。**さらにその内側で可視範囲（`mapSearchCenter` 中心から半径 × 1.3）に絞る**（絞らないと画面外のピンまで View 構築される = 実測 210 個/回 → 4 個/回。MU-1）。**burnt orange（黒 25% mix）にしない** — 訪問済みの茶と誤認される |
...
対処として色相を離す案（`.teal` / `.mint` = 茶から 160° 以上）も検討したが、**ティール = おすすめ が意味的に自明でない**ため不採用。カテゴリアイコン（カップ = コーヒーの店）は 3 種で共有したまま、★バッジで「推されている」という*状態*だけを足す方式を採る（Google Maps が人気スポットをカテゴリアイコンのまま強調するのと同じ発想）。**凡例は置かない** — マップの慣習で読める範囲に収めるのが前提で、凡例が要る意匠は意匠側の失敗とみなす。

**白フチの有無を混ぜない**。白フチは地図の情報密度から図形を切り離す効果が大きく、有無が混ざると数 pt のサイズ差を打ち消して意図しない序列が生まれる（訪問済みだけ白フチが無く、より小さいおすすめピンに負けて見えていた。教訓は [`tasks/lessons.md`](./tasks/lessons.md) 2026-07-27）。

共通チップ部品は `Components/TagChip.swift`（インタラクティブ `TagChip` + 凡例用 `TagLegendChip`）。画面ローカルにチップを再実装しない。`TagLegendChip` は production 上の使用箇所がゼロ（「好み一致」チップをタップ可能にした際に無くなった）。凡例が必要になったら再利用する。次に触るタイミングで用途が生まれていなければ削除してよい。

### マップ検索結果の提示（下部ドラッグシート）

マップの検索結果は「マップ主体 + 下部ドラッグシート一覧」で提示する（Apple/Google マップ風）。**検索バー直下にドロップダウンを出さない** — マップの主眼である位置を隠すため。
...
> 2026-09-19 まで一覧の 3 件目の後にインライン広告 1 枠を置いていたが、全画面下部固定帯（requirements §11-5）へ集約して撤去した。**この一覧に広告を戻さない**（下部帯と同時に画面へ出ることになる）。
...
**`.automatic` を使わない理由**（実装上の制約であり、見た目の好みではない）: おすすめピンの表示がカメラの可視半径に依存するため、**カメラ → ピン表示数 → カメラ** の自己駆動ループに入り、TestFlight で強制終了まで起きた。機序と実障害の詳細は `coding-conventions.md` §2.3 / lessons 2026-08-09。
...
画面最下部に広告専用の帯を常設し、**タブバーとコンテンツをすべてその上に置く**（requirements §11-5）。実装は `AppRootView` で `RootTabView` を **`VStack(spacing:)` で広告帯と縦に並べる** 1 箇所のみ（`Ads/AnchoredBannerAdView.swift`）。個々の画面は広告の存在を知らない。

> **`.safeAreaInset(edge: .bottom)` で包んではいけない。** `safeAreaInset` は**ビューのフレームを縮めず、セーフエリアの値だけを削る**。iOS 26 の浮動タブバーは `TabView` の**フレーム**基準で画面下端に位置決めするため、セーフエリアを削ってもタブバーは動かず、**不透明な広告帯の裏に隠れる**（2026-09-19 にこの実装で作って実機確認で発覚。lessons 同日）。フレームそのものを縮める `VStack` が要る。

守る規約:

- **`.errorToast` は広告より内側に適用する**。`ErrorToastModifier` は `.overlay(alignment: .bottom)` なので、広告帯より外側に付けるとトーストが広告に覆いかぶさる（AdMob のポリシー違反）。**「下部に重ねる」系の modifier を新しく足すときは同じ検討が要る**
- **広告帯に `.ignoresSafeArea(.keyboard, edges: .bottom)` を付ける**。付けないとキーボード表示時に広告がキーボードの上へ持ち上がる
- 広告ビュー自体は safe area 内（ホームインジケータの上）に置き、**背景だけ**画面下端まで伸ばす
- **タブバーとの間に最低 8pt** 空ける（誤タップ防止）
- **高さ 50pt を常時確保する。未受信・失敗時も畳まない** — 畳むと広告の到着でタブバーごとレイアウトが跳ねる。受信した広告は枠内に収める（`inlineAdaptiveBanner` の返す高さは 50 以下になりうる）。**インライン枠（`InlineBannerAdView`）は逆に畳むのが正**で、作法が分かれるのは意図的
- 全画面を覆う提示（`.sheet` / `.fullScreenCover`）は広告帯ごと覆う。**sheet 側に広告用の下端インセットを足さない**
- サイズは **`inlineAdaptiveBanner(width:maxHeight: 50)`**。**50 はこれ以上下げない** — `GADAdSize.h` が `maxHeight` を「32px 以上必須、**50px 以上を推奨**」と規定しており、50 が推奨下限ちょうど（32 まで下げられると読んで縮めないこと）。**配置はアンカード（下部固定）だが、サイズ計算にはインライン用の関数を使う** — 常設帯なので高さに天井が要る、という要件が先にあるため。アンカード用の関数は高さを指定できない（`currentOrientationAnchoredAdaptiveBanner(width:)` は `GADAdSize.h` の `#pragma mark Deprecated` 配下で非推奨、置換先の `largeAnchoredAdaptiveBanner(width:)` は上限が portrait height の 20% / 50-150pt で、実機で画面の約 15% を占めた）。**「アンカード帯なのにインラインサイズ」を不整合と見て戻さないこと**
...
- 写真の全画面表示を作るときは `.fullScreenCover` を使う（**未実装**。下記「写真表示」参照）
...
- **全画面表示（`.fullScreenCover` + ピンチズーム）は未実装**（`fullScreenCover` / `MagnificationGesture` とも実装が存在しない）。実装する場合の方針としてこの行を残すが、**現状の写真表示は一覧・詳細・カフェ詳細の 3 サイズのサムネイルのみ**。なお写真の保存解像度 2048px（[`requirements.md`](./requirements.md) §2 の写真フィールド行）は「全画面表示（6.7 インチ @3x = 1290px）にも余裕がある」ことを根拠の 1 つにしているため、**未実装でも解像度の前提としては生きている**
...
Places 写真を多く見せたい場合は、キャッシュではなく段階読み込み（「さらに表示」でユーザー操作を挟む）で対応する。
