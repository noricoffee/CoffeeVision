...
### 1.0.1 リリース（2026-08-21 起票）

> **機能追加なしのパッチ**。中身は「1.0 時点の既知の残務」（カテゴリ 1）からの 3 件 + 集客の 1 件で、**個別の詳細は各行が正本**（ここには複製しない）。`MARKETING_VERSION` / `versionName` は 2026-08-21 に `1.0.1` へ更新済み。
>
> **提出タイミングは実データ待ち**: 1.0 のリリースが 2026-08-21 のため、着手時点で Crashlytics / App Analytics に実ユーザーのデータが無い。**実装は先に積み、提出だけ 1〜2 週間後**に回してフィールドの発見を同じ 1.0.1 に合流させる（★1 レビューの初動対応にも使える）。バージョン番号は既に上げてあるのでこの待ちにコストはない。
>
> **Crashlytics の疎通は 2026-08-21 に TestFlight 実機で実証済み**（シンボル化まで。implementation_note 同日）。したがって**待った先でクラッシュがゼロなら「無事故」と読んでよい**（「計測が壊れていた」という可能性は潰してある）。
...
> **残るのは目視のみ**（[`tasks/verification-checklist.md`](./tasks/verification-checklist.md)）。提出は上記のとおり実データ待ち。
...
>
> **ユーザー数が少ない局面での優先順位（2026-08-21）**: 打ち手を選ぶ前に **ASC の App Analytics でインプレッション数と CVR を見て切り分ける**。「インプレッションが出ていない（露出の問題）」のか「出ているのに DL されない（ストアページの問題）」のかで対象フィールドが正反対になる。1.0 リリースが 2026-08-21 なので、数字が溜まるまでは断定しない。この局面で**優先度が下がるもの**は 2 つ:
> - **ASO-5 はリテンション施策**（既存ユーザーの再訪頻度を上げる投資）。分母が小さいうちは効果の絶対値も小さい。新機能なのでパッチにも載らず、1.1 の目玉に取っておく方が筋が通る
> - **ASO-4③ の A/B（Product Page Optimization）はサンプル不足で有意差が出ない**。無料だが実施するとインプレッションを消費する。動画・スクショ（ASO-4①②）を先に整えて、流入が増えてから回す
>
> 逆にこの局面で効くのは**露出とストアページ本体**: 下記「検索インデックスの空き枠」25 字 / ASO-4① App Preview 動画 / ASO-7① In-App Events。**いずれもバイナリ提出が不要**なので、1.0.1 のビルドを待たずに並行で動かせる。
...
| [ ] | ASO-7 | 低コストで拾えるもの: ① In-App Events（「今月のコーヒーふりかえり」で検索結果・カテゴリにイベントカードを露出）② 共有の App Store 導線（下記のとおり 2 段に分割）③ 小刻みなアップデート | ② の分割は 2026-08-21。①③ はバイナリ不要 |
