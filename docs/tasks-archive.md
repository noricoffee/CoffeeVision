...
#### マップ検索でテキスト検索が 0 件のときのフィードバック（2026-08-10 起票）

> ユーザー報告: マップの検索バーで検索して結果が 0 件でも何のフィードバックも出ない。原因は `MapSearchController.handleCompletion` が 0 件案内（`areaSearchEmptyMessage` → `errorToast`）を**「このエリアを検索」分岐にしか置いていない**こと。テキスト検索側は `fitCameraToSearchResults([])` が無反応で終わり、結果シートも `MapTabView.isShowingSearchResultsSheet` が「結果が空でない」を条件にするため出ない。既存のトースト経路をテキスト検索と共有する形に一般化する。

| 状態 | タスク | 備考 |
|------|--------|------|
| [x] | **トーストの表示位置を上部 → 下部（タブバーの上）へ**（ユーザー指摘 / 全画面一括）→ **2026-08-10 完了**（フラグ無し `** BUILD SUCCEEDED **` / **シミュレータ目視確認済み**）。`ErrorToast.swift` の `overlay(alignment:)` / スライド方向 / スワイプ消去の向きの 3 点。上部は `MapTabView` の検索バーと同じ位置（双方 `.padding(.top, 8)`）で、0 件案内が検索欄を 4 秒間覆っていた | `.errorToast` 適用 8 箇所すべてに波及。**配置の引数化は不採用**（画面ごとに位置が変わる方が悪い）。ui-ux-guidelines「エラー表示」を追随済み |
| [x] | テキスト検索 0 件でもトーストを出す → **2026-08-10 完了**（フラグ無し `** BUILD SUCCEEDED **` / **シミュレータ目視確認済み**）。`areaSearchEmptyMessage` を `emptyResultMessage` に一般化し、`handleCompletion` の非エリア検索分岐でも 0 件時にメッセージを立てる。文言はエリア =「このエリアにカフェが見つかりませんでした」/ テキスト =「「〈クエリ〉」に一致するカフェが見つかりませんでした」 | iosApp 完結。**0 件でも結果シートを出す案は不採用**（空シートが 180pt 占有し現在地 FAB も押し上がる）。判断は implementation_note 2026-08-10 |
...
>
> **SR-4**（レビュー #2）は「`CallbackFlow` にエラーチャネルが無く Firestore の `permission-denied` が握り潰される」。**調査すると指摘は表層で、根本は同期購読の `Job` を誰も保持していないこと**だった（`AppContainer.startInitialSync()` が戻り値を捨て、`resetAndRebootstrap()` もブリッジしか止めない）。Rules は `request.auth.uid == uid` しか許さないので、**サインアウトのたびに旧 uid の購読が 1 組ずつ残り、必ず権限エラーになる** — つまり「握り潰されていたエラー」は事故ではなく**自分で作って自分で捨てていた**。修正は「ライフサイクル管理（根本）」+「エラー伝播（表層）」の 2 段。
>
> **SR-5**（レビュー #7）は「`LocationManager()` が View struct の init ごとに作られ `requestLocation()` を誘発する」。**計測したら指摘の前半（再生成）は起動時には再現せず（init は 1 回）、後半（GPS 誘発）が実在した** — `manager.delegate = self` の代入で `locationManagerDidChangeAuthorization` が発火し、ハンドラが無条件に `manager.requestLocation()` を呼ぶため、**生成 = GPS 取得 1 回**になっていた。実測で起動 1 回あたり GPS 要求 2 回 → 1 回。
...
| [x] | SR-4 | **Firestore 同期の Job ライフサイクル管理 + Flow のエラー伝播**（レビュー #2）→ **2026-08-09 完了**（KMP テスト 2 件追加 / 全モジュール iOS 505 件 PASS / Android `assembleDebug` OK / フラグ無し `** BUILD SUCCEEDED **` / **ブリッジ挙動をシミュレータで実測**）。①KMP: `AppContainer` が `coffeeSyncJob` / `savedCafeSyncJob` を保持し `stopSync()` を公開、`startInitialSync()` は冒頭で `stopSync()` を呼び冪等化 ②Swift: `AppState.resetAndRebootstrap()` の冒頭で `container.stopSync()` ③Swift: `CallbackFlow` / `CallbackFlowOptional` の `onStart` に `fail:` を追加し、completion handler を 1 回だけ呼ぶ `FlowCompletionGate`（`OSAllocatedUnfairLock`）を経由 ④KMP: `startSync` が終端例外を catch して同期だけ止める（**リトライしない** — `permission-denied` は非一時的）⑤両 `RemoteXxxDataSource` インターフェースにエラー契約を明記 `shared/core` + `shared/domain` + `iosApp` | **根本は表層指摘の外にあった**（同期 Job のリーク）。**`fail` の `NSError` は Kotlin 側で `NSError-based exception` になり catch 可能 = fatal ではない**ことを DEBUG 限定フックの PoC で実測（フックは検証後削除。生成ヘッダの「uncaught Kotlin exceptions are fatal」は逆方向の注意書きだった）。ついでに `observeAnalyticsConsent` がエラー時に `false` を emit していた**値の捏造**も是正。**残る窓**: signOut 完了 → `resetAndRebootstrap()` までの間は旧リスナが生きており権限エラーが 1 回届きうる（④で無害化。完全に閉じるにはサインアウト経路の再構成が要るためスコープ外）。教訓は lessons 2026-08-09。**シミュレータでの実操作確認済み**（2026-08-09、ユーザー: サインアウト → 新 uid での同期再開） |
| [x] | SR-5 | **`LocationManager` の生成が位置取得を誘発しないようにする**（レビュー #7）→ **2026-08-09 完了**（**シミュレータで修正前後を実測** / フラグ無し `** BUILD SUCCEEDED **`）。`hasPendingRequest` を追加し、`locationManagerDidChangeAuthorization` が `manager.requestLocation()` を呼ぶのは「`requestLocation()` が権限ダイアログ待ちで保留していたとき」だけにした。呼び出し側（`MapTabView+Location.swift` / `CoffeeEditorView.swift`）は**無変更** `iosApp/Utilities/LocationManager.swift` | **指摘の前半は再現せず、後半が実在した**: 起動時の `init` は 1 回（`RootTabView.body` は `mapSearchCenter` を読まないため `@Observable` の粒度で再生成されない）。一方 `manager.delegate = self` の代入が callback を発火させ **GPS 要求が起動 1 回につき 2 回**走っていた（実測 → 修正後 1 回）。実害は無駄な GPS より**両画面の明示ガードの迂回**（`if !didSetInitialCamera` / 「未許可・未決定は何もしない」要件 2-8）。権限 3 経路（未決定→許可 / 許可済み / 未決定→拒否）を `simctl privacy` で**自動実測して全て確認**。**シミュレータでの実操作も確認済み**（2026-08-09、ユーザー: マップのパン / FAB recenter / 記録エディタの現在地サジェスト）。教訓は lessons 2026-08-09 |
...
>
> **2026-08-07 に 8 件すべて決着**（完了 5 / 取り下げ 3）。取り下げは UX-2（ピン重なり = 実測で間引きが情報の 84% を捨てると判明）/ UX-5（閉店の赤 = Google マップの慣習に従う）/ UX-6（評価分布のビン欠落 = ユーザー判断で不要）。その後 **UX-9**（UX-1 の意匠を概念ピン 4 種へ拡張）を追加で実施。**残務は 2 件**（①は 2026-08-09 に解消）: ~~①App Store スクショ（記録一覧 / マップ）の再撮影~~ → **2026-08-09 に 6 枚とも再撮影し `screenshots/submit/` を再生成済み**（記録一覧 = ナビバー右上の `+`、マップ = バッジ付きカップピンを目視確認）。同日に**表示順も分析 2 枚を先頭へ組み替え**、ファイル名をリナンバーした（app-store-metadata §5）②営業状態の文言割れ（`CafeDetailView`「営業時間外」/ `CafeSelectionCard`「終了」）③`MKLocalSearch` が 50 件で頭打ちな件と、名前ヒューリスティック / ネガティブキャッシュが渋谷で 1 件も除去していない件（いずれも UX-2 の計測で判明。未調査）。
>
...
| [x] | UX-3 | **追加アクションの位置を統一**。記録リストの右下 FAB を廃し、カフェ詳細と同じ `ToolbarItem(.topBarTrailing)` の `plus` へ寄せる。規則は ui-ux-guidelines「追加アクションの配置」 | **`04-record-list.png` の再撮影は 2026-08-09 に完了**（提出用の `screenshots/submit/` も再生成済み）。空状態の「右下の + ボタンか…」の文言も追随が要る |
...
| 状態 | ID | タスク | リスク |
|------|----|------|--------|
| [x] | MU-1 | `displayedCuratedCafes`（`MapTabView+PinResolution.swift`）に**可視領域フィルタが無い**。ズームゲート（可視半径 3000m 以内）を通ると `curatedCafes` 421 件を件数で絞らずそのまま返すため、**画面外のピンまで Annotation に載る**（実測でピン body 評価 210 個/回） | 2026-08-09 完了。**基準は `latestVisibleRegion`（可視領域の矩形）ではなく `mapSearchCenter` からの距離（マージン 1.3 倍）を採った** — `latestVisibleRegion` は現在 body から読まれておらず `onMapCameraChange` で無条件代入されているため、body で読むと**新しいカメラ依存を作る**（`MKCoordinateRegion` は `Equatable` 非準拠で同値ガードも書きにくい）。`mapSearchCenter` はズームゲートで既に読んでおり `isEquivalent` の同値ガードも入っている。**実機・クリーンインストール + 許諾フローで実測**: curated ピン **210 個/eval → 4 個/eval**、body 再評価 **589 回/60秒 → 9 回/60秒**、引き金は 8 種すべて各 1 回、`cam` も収束、60 秒間プロセス生存（`devicectl` 起動は watchdog 有効）。**発散を復活させていないことを確認済み** |
| [-] | MU-2 | 全ピンが `NavigationLink` / `Button` でラップされており、ピン 1 個 = `_UIHostingView` 1 個 + `ButtonBehavior` の `State` 初期化になっている（クラッシュログのスタックに `WrappedButtonStyleBody` / `initializeWithCopy for NavigationLink` が出ていた）。`NavigationLink(value:)` が保持する `CafeDetailRoute.initialCafe` は Kotlin の `Cafe` オブジェクトで、更新ごとに ObjC 参照カウントのコピーが走る | **2026-08-09 取り下げ**（MU-1 の実測を見てユーザー判断）。curated が 210 → 4 個/eval になり、この規模では `NavigationLink` の値コピーを削っても実測・体感に現れない。加えて **`Button` に変えても `ButtonBehavior` の `State` 初期化と `_UIHostingView` 1 個/ピンという本体コストは変わらない**ため、消えるのはルート値のコピーだけ。遷移方式の変更は影響範囲が広く投資に見合わない。再燃する条件は「ピン数が再び 3 桁に戻る」= curated の対象県を大幅に増やしたとき |
...
| 状態 | タスク | 備考 |
|------|------|------|
| [x] | 上記 3 エントリを `## 2026-08-09` セクションへ移動し、「注:」ブロックを削除する | 2026-08-09 完了。**起票時の前提が誤っていた**: 「索引の日付参照も `(08-08)` を指している」と書いたが、`grep -n "(08-08)"` の結果は **0 件**で、移動対象 3 エントリは**そもそも索引に未登録**だった（付け替えではなく新規追加が必要だった）。移動と同時に索引へ 3 行追加（SKIE / Coroutines / iOS SwiftUI の各主題行）。検証: `### ` エントリ数 117 → 117 で欠落なし、全 117 件について「発生源の日付」と「所属セクション」の一致を機械チェックして**不一致 0 件** |
| [x] | **索引に日付参照が 1 件も無いセクションが 5 つある**（上の作業で発見し全期間を点検した）。前文は「新規 lesson を追記したら索引にも 1 行足す」と定めているが守られていない。**07-24（1 件）/ 07-27（1 件）/ 07-31（1 件）/ 08-01（4 件）/ 08-08（7 件）= 計 14 エントリが未登録** | 2026-08-09 完了。14 件すべてに索引行を追加し、**参照が 0 件のセクションは 0 になった**（機械チェックで確認）。振り分けは xcconfig / Places 1 件（`locationBias`）、iOS / SwiftUI 4 件（輪郭の有無 / `.onChange` の `Task` / `@concurrent` 付け忘れ / enum の private func）、SKIE / KMP 1 件（ブリッジ層の残骸）、プロセス / 設計 6 件（doc を開かず判断 / `--include` 取りこぼし / 数え上げ / 最適化前に数える / 派生 doc を辿る / 「対象外」の根拠）、ビルド成功⇄実態の乖離 2 件（pbxproj の無言上書き / `xcodebuild` 引数の SPM 波及）。挿入位置は索引の日付昇順を維持した。**残る課題**: 索引は 1 参照に複数エントリをまとめる場合があるため（例 `(06-25、07-03 再発)`）、参照数と本文エントリ数の差がある日付（06-02 は 7 対 2、07-03 は 4 対 1、08-06 は 6 対 1 等）は、まとめ記載か真の欠落かをエントリ単位で確認しないと判別できない。**「0 件のセクションが無い」= 全エントリが引ける、ではない** |
...
| 状態 | タスク | 備考 |
|------|------|------|
| [x] | **Places API キー悪用時の請求上限を作る**（2026-08-11 起票・完了 / **提出後**）。Cloud Billing の**予算アラート** + Places API (New) の**分あたりクォータ上限**をユーザーが設定 | **公開された瞬間にキーは抽出可能になる**ので、本来は承認前に閉じるべき窓。方針自体は implementation_note 2026-07-08 で「バンドル ID 制限はヘッダなりすましで突破可能 → 現実的な守りは予算アラート + クォータ上限」と決めていたが、**実施したかの記録がどこにも無く**、提出後レビュー（2026-08-11）で未確認と判明した。**実値は Cloud Console が正本**（doc に書かない）。詳細と判断根拠は paid-services §1「コスト抑制の現状」。**`per user` クォータは無制限のまま**（2 つは AND 評価なのでプロジェクト × API 単位だけで請求の天井は成立する。`per user` が買うのは天井ではなく発信元の隔離だが、API キーのみで OAuth ユーザー identity を持たず Google 側の「user」識別が確実でないため依存しない）。**API キーの「API の制限」も Places に限定済み**（2026-08-11）— 未制限だと他 Maps API へ迂回されクォータ上限が無意味になるため、**この 3 点（予算アラート / クォータ上限 / API 制限）が揃って初めて天井になる**。副作用として、新しい Google API を使い始めるときは許可リストへの追加が要る |
| [x] | **Remote Config に `review_prompt_enabled` を実在させる**（2026-08-11 起票・完了 / **提出後**）。Boolean / 既定値 `true` / 条件なしで作成・公開 | ASO-1 のレビュー依頼キルスイッチ。実装（`ReviewPrompt.swift`）は `setDefaults` を持たず **キー未設定時は `source == .static` → `true`（発火する）** に倒してあるため、**止めたくなったときにキーが無いと止められない**状態だった。`true` で作っても挙動は不変（`.static` → `.remote` になるだけ）で、目的は「`false` に変えるだけで止まる状態」を先に用意すること。⚠️ **即時停止はできない** — `minimumFetchInterval` は SDK 既定 12h かつ fetch は起動時 1 回（`RemoteConfigBootstrap`）なので、反映まで**最大 12h + ユーザーの次回起動**。即時性が要るなら間隔短縮 = 新ビルドの提出が必要 |
...
## 付録: Phase 1〜初期の PR 振り返りログ（旧 `tasks/pr-log.md`）

> 2026-06-25 の記録を最後に凍結済み（2026-07-22 判断）。以降は同じ役割を本ファイルの完了セクション + `implementation_note.md` + git log が担う。当時のフォーマットのまま収録する。**旧ドメインモデル（Visit / CoffeeItem / FoodItem）時代の記録**を含む — 現行モデルは 2026-06-19 のフェーズ 7 で `CoffeeRecord` 主体に全面置換済み。

Phase 1〜初期の PR・バグ修正・機能追加の振り返り記録（当時のフォーマット）：
