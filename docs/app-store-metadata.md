...
> ⚠️ **`04-record-list.png` は再撮影が必要**（2026-08-07 時点で未実施）。撮影時点では右下に FAB があったが、UX-3 で追加ボタンをナビバー右上へ移したため**現物と一致しない**。`screenshots/6.9/` の原本と `screenshots/submit/` の焼き込み版の両方を撮り直すこと。**`01-map.png` も再撮影が必要** — UX-1（おすすめピンに★バッジ追加）に続き UX-9 で**保存済み / 好み一致ピンの本体アイコンがカップに変わりバッジが付いた**ため、この枚が主題としているピン 3 種のうち 2 種が現物と一致しない。残り 4 枚は UI 変更の影響を受けていない。
