...
- **プロセス / 設計 / 診断の姿勢**: 横断 doc は構造的に陳腐化 (06-16) / テストダブルの接続を本番と乖離させない・dev シードが本番バグをマスク・KMP テスト実行の環境メモ (07-03) / 無音フォールバックは偽バグ報告になる (07-06) / 地図の「表示」と「解決」の集合ズレ (07-08) / 外部 SDK の required 判定はクライアントで制御不能 (07-14) / レイアウト実測の再入ガードが過渡値を破棄 (07-15) / ドメインのフィールドが写る先は 5 経路・export DTO が抜けやすい (07-25) / background dispatch 中の `git add -A` はコミットを混ぜる (07-25) / 永続ストアを増やしたら削除経路にも足す・消し漏れはコードに現れない (08-06)
...
- **症状**: 「ASC でアプリアイコンが表示されない」というユーザー報告に対し、親が「ソース PNG のアルファが原因」と当たりをつけ、`ios-engineer` へ **「検証はビルド成果物側で行う。`sips -g hasAlpha <app>/AppIcon60x60@2x.png` が `no` になること。**ここが本丸**」**と指示して dispatch した。サブエージェントが実測したところ、この派生 PNG は **Debug ビルドでは新旧どちらのソースでも `hasAlpha: yes`**、**Release / Archive では新旧どちらでも `no`** で、**ソースの修正と完全に無関係**な値だった。親が指定した検証手段そのものが成立していなかった
...
- **発生源**: 2026-08-06、ASC のアイコン非表示調査。**同日 2 件目の検証手段ミス**（1 件目は `plutil -extract` の `-o` 省略でファイルを破壊）。どちらも「検証しているつもりで、観測対象が壊れている / 無関係」という同じ形
- **未解決として残ったこと**: アルファ除去自体は Apple の要件に素直な正しい対応だが、**それが「ASC で表示されない」症状の原因だった確証は取れていない**。アルファ付きのまま TestFlight run #23 が `success` しており `ITMS-90717` も出ていなかった。ASC の「App 情報」ページのアイコンは提出済みバージョンのものを表示するため、**審査に出す前は空欄が正常**という可能性が残っている（ユーザーに表示場所を確認中）。**原因未特定のまま「直った」と結論しないこと**
