...
|---|---------|------|------------|--------------|
| 1 | `01-map.png` | マップ画面 | 訪問済み / 好み一致 / 保存済みのピン + 埋め込み検索バー | お気に入りの一杯を／発見しよう |
| 2 | `02-analyze-summary.png` | 分析画面（サマリ） | 総杯数・平均評価 + 評価分布 + テイスティングレーダー | 味覚の輪郭が、／見えてくる |
| 3 | `03-analyze-suggest.png` | 分析画面（傾向と提案） | 4 軸の好み傾向 + 好みの豆の言語化 + 未体験豆の提案 | 好みから、／次の一杯が見えてくる |
...
> **2026-08-09 に 6 枚すべてを再撮影し、`screenshots/submit/` も再生成済み**。2026-08-07 に挙げていた「`04-record-list.png`（UX-3 で追加ボタンが FAB → ナビバー右上）/ `01-map.png`（UX-1・UX-9 でピンがカップ + バッジに）が現物と一致しない」問題は解消を確認した（04 = ナビバー右上の `+`、01 = ★ / ハート / ブックマークのバッジ付きカップピン）。
>
> **上部の写り込みに注意**: 初回の `02-analyze-summary.png` は**ダイナミックアイランドが黒いピルとして写り込んでいた**（原本の座標 (660, 80) が `rgb(0,0,0)`。他 5 枚は同座標が背景色）。ストアでは 6 枚が横並びで見えるため 1 枚だけ上部に黒い切り欠きが出る。同日中に 02 を撮り直して解消済み（現在は全枚が背景色）。**撮影後は 6 枚の同座標を並べて比べる**のが検出手段（1 枚だけ見ても「そういうものか」と流れる）。
...
>
> トーンは**両端が呼びかけ型**（#1「発見しよう」/ #6「チェック」）で**中の 4 枚が描写型**（「見えてくる」「積み上がっていく」「5 つの軸で」）。意図的な構成。
...
>
> **落とした 2 枚の副作用**: ①写真添付を写す枚がなくなった（記録詳細が唯一だった）②「行きたい」保存の訴求は `01-map.png` の保存済みピンのみが担う。将来 ASO-4（ストアページの CVR 投資）で枚数を増やすなら、この 2 つが最初の候補。
...
| 2026-08-09 | **スクリーンショット 6 枚を再撮影し `screenshots/submit/` を再生成**（1320×2868 / アルファ無しを全枚検証）。§5 の「`01-map.png` / `04-record-list.png` が UX-1・UX-3・UX-9 の UI 変更と一致しない」警告を解消済みとして消し込み、§10 のチェック行にも反映。焼き込みコピーは変更なし。**`02-analyze-summary.png` のみダイナミックアイランドが黒く写り込んでいた問題**（他 5 枚は同座標が背景色）は同日中に 02 を撮り直して解消し、検出手段（6 枚の同座標を並べて比べる）を §5 に残した。あわせて 2026-08-06 行の位置を日付順に是正 |
