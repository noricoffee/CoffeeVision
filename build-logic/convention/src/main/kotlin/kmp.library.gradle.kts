 ...
 * - Android 側 jvmTarget は 11（既存 sharedLogic と揃える）
 *
 ...
 *
 * 注: `jvmToolchain(17)` は **付けない**。既存 `sharedLogic` も付けておらず、開発機の JDK を
 * そのまま使う運用。toolchain を強制すると Gradle が JDK 17 のダウンロードを要求するため。
 */
plugins {
