...
- **`List` / `Form` の中では `tint` を明示しても `Label` のアイコンだけ効かない**（`accentColor` で描かれる）。`.tint()` は塗り・枠線には効くので**「効いていないわけではない」状態になり切り分けが難しい**。文字とアイコンの色を揃える必要があるなら、`tint` からの継承に頼らず **`Label` に `.foregroundStyle` を明示**する（状態ごとに色が変わるなら、ラベルを `let` で使い回さず分岐ごとに構築し直す）。**コンテナが子の見た目を書き換える型の挙動は子側のコードを読んでも分からない**ので、色が揃わないときは同じ View を `List` の外に置いた対照を実際に描画して比べる（lessons 2026-08-07 続報）
  - **`tint` を指定しない `.borderedProminent` と組むと、色がズレるのではなく `Label` のアイコンが消える**。塗りの背景が `accentColor`、アイコンも `accentColor` になり同化するため（テキストは白前景で残るので「アイコンだけ無くなった」と見える）。**「塗り + `Label` + `List` / `Form` の中」の 3 条件が揃ったら `.foregroundStyle(.white)` を必ず当てる**。この罠は**知っていても踏む** — 実際 2026-08-31 に、罠を検証項目として明記した上で、対処済みの実例（`CafeDetailView.saveButton`）の 200 行下に同じバグを作った。**既知の罠は「後で目視する項目」ではなく「いま書くコードの条件判定」として使う**（lessons 2026-08-31）
