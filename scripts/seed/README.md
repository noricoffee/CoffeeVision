...
- **大手チェーンは人気枠を必ず通過する**（レビュー数が桁違いのため）。名前キーワードで落とすしかないので、**新しい県を足したら必ずチェーン混入をチェックする**（2026-07-28 の 9 県展開では 33 件混入した）。部分一致なので語の選定に注意 — 例: `珈琲館` を入れると独立店の `表参道珈琲館` を巻き添えにする。`STARBUCKS RESERVE(R) ROASTERY TOKYO` はカタカナ「スターバックス」だけを登録することで意図的に残している（ラテン表記の `starbucks` は入れない）
