...
| [~] | UX-4 | **カフェ詳細の未保存「保存する」がグレーで無効に見える**。原因は `.bordered` に `.tint` が無く `accentColor`（茶）を継承していたこと。`.tint(.indigo)` を付けて色セマンティクス表に合わせる。**塗りの有無で状態を表す設計（保存済み = 塗り）は維持**（アイコンの `bookmark` / `bookmark.fill` と同じ「塗り = ON」の語彙。強調を反転する案はユーザー判断で不採用） | `CafeDetailView.swift:178-205` |
