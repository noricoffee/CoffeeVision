...
追記（同日）: コーヒー記録タブの固定広告を**下部 → 上部**（ナビ / 検索バー直下の `.safeAreaInset(edge: .top)`）に変更（ユーザー確定）。理由: ① FAB と広告の近接（16pt）は誤タップを誘発し AdMob ポリシー上もリスク ② タブバー / 広告 / FAB の下部 3 段渋滞の解消 ③ 広告の畳み挙動で FAB 位置が動く副作用の解消。分析タブは FAB がないため下部のまま（**上下非対称は意図的**）。FAB は従来の `overlay(alignment: .bottomTrailing)` に戻しシンプル化。`AnchoredBannerAdView` は上下どちらの safeAreaInset にも載る汎用コンポーネント。
