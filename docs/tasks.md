...
>
> **2026-08-31 に審査提出**（ユーザー実施）。**スクショ・What's New とも積み残しを抱えたまま出している**（下表参照）。審査通過後は `itunes.apple.com/lookup` で `version` / `currentVersionReleaseDate` を確認する（1.0.1 と同じ手順）。
...
| [ ] | **What's New の版管理を決める** | **1.0.2 は 1.0 の文言をそのまま提出**（1.0.1 も同様）。結果「`CoffeeVision を初めてリリースしました。`」が 1.0.2 のリリースノートとして表示される。app-store-metadata §9 を「実際に提出した文言の記録」として整理済みで、**版ごとに書き下ろすか 1 本を使い回すかが未決** |
