...
| [ ] | 検証: 保存 → ピン表示 → 解除の round-trip、Firestore コンソール確認、既存ピンとの共存・フィルタ切替、一覧シートのスワイプ解除・記録ありバッジ | `verify-kmp-ios` skill に従う。**シミュレータ / 実機での目視はユーザー作業**。新テーブル追加（migration 3.sqm）だが既存端末は migration で更新されるため再インストール不要の想定（要確認） |
