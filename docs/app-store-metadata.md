...
> カテゴリは 2026-07-22 確定（プライマリ = フード/ドリンク、セカンダリ = ライフスタイル）。SKU は社内識別子のため下書きのまま可。サブタイトルは 2026-07-12 確定済み。
...
```
カフェ,コーヒー,珈琲,喫茶,記録,日記,カフェ巡り,コーヒー記録,テイスティング,焙煎,ドリップ,スペシャルティ,豆,抽出,ラテ,エスプレッソ,カフェマップ,ハンドドリップ,自家焙煎,カプチーノ
```

（2026-07-22 最終レビュー: **20 語・98 字**で確定。余裕枠にハンドドリップ / 自家焙煎 / カプチーノを追加。文字数は `len` 実測で 100 字以内を確認）
...
### 確定が必要な原稿系（ユーザー判断）
- [x] サブタイトル文言の確定（2026-07-12 確定「コーヒー記録・分析・行きたい店」/ 15 字）
- [x] プライマリ / セカンダリカテゴリの確定（2026-07-22: フード/ドリンク + ライフスタイル）
- [x] 説明文・キーワードの最終レビュー（2026-07-22: 全原稿を `len` 実測で上限内確認。アプリ名 12 / サブタイトル 15 / プロモ 107 / 説明文 1013 / キーワード 98 字。キーワードは 3 語追加で 20 語）
