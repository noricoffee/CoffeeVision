...
6. `AppContainer.kt`: primary constructor + **全セカンダリコンストラクタ**（本プロジェクトは 2 本ある: iOS 用 = `coffeeInsightProvider` を取る / Android 用 = 取らない、で区別する）に `val fooRepository: FooRepository` を追加。KDoc 内の「Swift 側の呼び出しシグネチャ」コメント文字列も両方更新すること（`.h` で裏取りする前提の記述なので更新漏れは実害小さいが一貫性のため直す）
