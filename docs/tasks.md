...
| 状態 | タスク | 備考 |
|------|--------|------|
| [~] | UX-1 | **おすすめピンを★バッジで差別化**。`cup.and.saucer.fill` を共有したまま右上に `star.fill` の白地バッジを乗せ、サイズ 34 → 32。色相実測と不採用案（`.teal` 化 / 凡例追加）の根拠は ui-ux-guidelines「おすすめピンを色で区別しない理由」 | iosApp 完結。`MapPins.swift` の `CuratedCafePin` |
| [ ] | UX-2 | **ピンのクラスタリング**。渋谷のような密集地で 5〜6 個が重なり、下のピンがタップ不能に見える。ラベル衝突も併発 | 未設計。着手前に方式（クラスタ / スパイダー展開）を確定させる |
| [ ] | UX-3 | **追加アクションの位置を統一**。記録リストは右下 FAB、カフェ詳細は右上 `+` で 2 通り。FAB は iOS 非標準かつ最下行を隠している | HIG に寄せるならナビバー右上へ統一 |
| [ ] | UX-4 | **カフェ詳細の「保存する」がグレーで無効に見える**。この画面の主目的なのに、茶色の電話番号リンクより視覚的優先度が低い | `CafeDetailView.swift` |
| [ ] | UX-5 | **「営業時間外」の赤を外す**。iOS で赤はエラー / 破壊的操作の色。単に閉店中なだけの情報を警告として提示している | 同上 |
| [ ] | UX-6 | **評価分布ヒストグラムのビン欠落**。x 軸が 3 / 3.5 / 4 / 4.5 / 5 のみで 1〜2.5 が省かれ、「低評価が無い」のか「軸が無い」のか判別できない。軸を 1〜5 固定に | `AnalysisView+Statistics.swift` |
| [ ] | UX-7 | **記録リストに写真サムネイルが無い**。写真を撮れるアプリなのに一覧がテキストのみで、想起の手がかりが弱い | `CoffeeListView.swift` |
| [ ] | UX-8 | **記録リストの主従を再検討**。カフェ名が主・コーヒー名が副のため、同一カフェが連続する箇所で何を飲んだかを目で追いにくい | 主従の入れ替えか併記かは着手時に決める |
