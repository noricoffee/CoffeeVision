...
- **SKIE / KMP ブリッジ（Swift⇄Kotlin）**: SuspendInterop は呼び出し方向限定 (06-04) / デフォルト引数を引き出さない (06-04) / EnumInterop enum→@frozen・camelCase (06-09) / SKIE 型は `.swiftinterface` を見る (06-09) / `KotlinDouble?` を `String(format:)` 直渡しで 0.0 (06-21) / 撤去の残骸はブリッジ層（Swift 公開面）が「宣言 1・参照 0」に見えて数え漏れる (07-31) / `nonisolated` は宣言単位で効き同じファイルの `private` ヘルパ型には伝播しない (08-09)
...
- **ビルド成功⇄実態の乖離（SourceKit / DerivedData / 偽成功）**: `No such module` が実ビルドと乖離 (06-08) / DerivedData 古い symlink (06-09) / `OVERRIDE_KOTLIN_BUILD_IDE_SUPPORTED=YES` は偽の成功 (06-19) / 実機バイナリは `grep -a` で確認 (06-23) / JVM green・iOS だけコンパイル不能の 2 種 (07-07) / `project.pbxproj` の `buildSettings` は同名の xcconfig を無言で無効化（ビルド設定は解決後の値で確認する）(08-08) / `xcodebuild` の引数は SPM 依存パッケージ全体にも適用される（計測の細工が計測対象を変える）(08-08)
...
- **xcconfig / Places / API キー / シリアライズ**: 既存 xcconfig の継承漏れ (06-10) / フォールバック宣言は `#include?` の前 (06-23) / REST DTO `emptyList` + `expectSuccess=false` で握り潰し・`encodeDefaults=false` で default フィールド脱落 (06-23) / `locationBias` は範囲制限ではなく近傍ヒント — 範囲内限定 UI はクライアント側フィルタが要る (07-24)
...
**再発の実測（2026-07-25 / 2026-07-27 の定期棚卸し）**: 上記の再発防止策を入れた後も陳腐化は出続けており、**「同 PR で横断 doc も直す」だけでは防ぎきれていない**ことが 2 回の棚卸しで確定した（第 1 巡 = data-model 6 件 + kmp-bridge 6 件 + architecture 3 件、第 2 巡 = 8 doc で 10 件）。第 2 巡の 10 件は**全件が「別の doc だけが追随して残りが取り残された」形**で、単発の書き忘れではない:

- 広告の提示先が上部ドロップダウン → 下部シートへ移った 2026-07-22 の変更で `requirements.md` §11-2 は当日更新されたが `paid-services.md` §2b が残った
- `iosApp` のディレクトリは `Ads/` 追加・`Bridge/` 廃止・`Utilities/` 新設と何度も変わったが `coding-conventions.md` §2.2 のツリーは初期のまま（実在しないディレクトリ 3 つを案内していた）
- root `README.md` の `core` は**一度も実在しなかった**構成（Result / Logger / Dispatchers）で、依存の矢印も向きが逆だった。`architecture.md` は第 1 巡で直っていたので、README だけが分割前の想像図で残っていた

**結論として、事前の同 PR 追随（予防）と定期的な Phase 1（事後の受け皿）は両方要る**。「変更のたびに、この事実を書いている doc は他にどれかを引く」コストが実際には払われないので、`curate-doc` skill の Phase 1（コード突き合わせ）を doc ごとに一巡させるのが唯一確実な検出手段になっている。**行数閾値のフックは棚卸しの起動条件としては不十分**（第 2 巡で閾値を超えていたのは 8 doc 中 0 本 — 陳腐化は行数と相関しない）。
...
- **横展開点検（2026-07-27）**: ①フチ表現の同型: `grep -rn "stroke(Color(.systemBackground)" iosApp/iosApp --include="*.swift"` → **`MapPins.swift` の 6 ピンのみ**（MP-1 / MP-2 で全件統一済み）。`grep -rn "\.stroke(" iosApp/iosApp --include="*.swift"` の他ヒットは `TastingRadarChart` のグリッド線・データ線 3 箇所で用途が別。②影の不揃い: `grep -rn "\.shadow(" iosApp/iosApp --include="*.swift"` → 意味ピン 4 種が自色 `opacity(0.4)` / `radius: 4` / `y: 2` で揃っているのに対し **`CuratedCafePin` だけ `opacity(0.5)`** が残る（同じ「体系の中で 1 つだけ強い」形。ただし ui-ux-guidelines の curated 定義は「サイズと彩度で強調」であり影は仕様外の暗黙強調）→ **同日 MP-3 で 0.4 へ統一**（MP-2 の目視後にユーザー判断）。`AppleNearbyCafePin` の `.black.opacity(0.25)` / `radius: 3` は低強調ピンの意図的な例外で規則に明記済み。③チップ体系（`TagChip` / `TagLegendChip`）は `TagLegendChip` の production 使用箇所が 0 のため不揃いの実害なし

## 2026-07-31

### 機能を撤去した残骸のうち、ブリッジ層のものは Kotlin 側 grep で「宣言 1・参照 0」に見えて数え漏れる

- **症状**: エディタの産地サジェスト（2026-07-22 撤去）のデッドコードを掃除する際、事前調査で「`AppContainer.beanProfileMatchUseCase` と `BeanProfileRepository.getByOrigin` の 2 つ」と数えたが、実際に着手すると `shared/framework` の `AppContainer.fetchBeanSuggestions(origin:processing:)` が 3 つ目として出てきた。**撤去から 9 日間、公開 API として xcframework に載り続けていた**
- **原因の構造**: 最初に見つけた 2 つがどちらも「repository のメソッド」「container のプロパティ」というドメイン寄りの層にあったため、その層だけを見て数えた。ブリッジ層（`shared/framework` の `AppContainer` 拡張）は **iOS からしか呼ばれない前提で作る**ので、Kotlin 側の grep では常に「宣言 1 件・参照 0 件」に見える。生きている `makeMapViewModel` も死んでいる `fetchBeanSuggestions` も Kotlin 側からは同じ見え方をするため、**参照数では生死を判定できない**。加えて `getByOrigin` の KDoc は撤去当時に「本番コードに呼び出し元が無いため実害はないが…」と自分が死んでいることを明記しており、**デッドコードだと分かった時点で消さずに注記へ落としたため、注記ごと残り続けた**
- **修正パターン**: 撤去の残骸を数えるときは層で区切って全部見る。① ドメイン（interface のメソッド）② 合成ルート（`AppContainer` のプロパティ）③ **ブリッジ（`shared/framework` の `AppContainer` 拡張・`@Throws` 付き public 関数）** ④ プラットフォーム実装（`androidMain` の override / iosApp の Swift 実装）。生死の判定は Kotlin 側の参照数ではなく **`iosApp` / `androidApp` を含めた呼び出し元の有無**で行う。削除の順序は **KMP → iOS**（Swift はプロトコル要件に無い余分なメソッドがあってもコンパイルが通るが、逆順にすると要求メソッドが未実装になって壊れる = **プロトコル要件を減らす変更は要件を出す側から先に**）
- **教訓**: **「呼び出し元ゼロ」を見つけたら、その場で消すか tasks に項目を立てる。KDoc に注記して残さない**。注記は次に読む人に「知った上で残している」と伝えるだけで、消す動機を作らない。そして**撤去作業の見積もりは、ブリッジ層を数え直してから出す**（Swift 公開面はコンパイラも Kotlin 側 grep も守ってくれない唯一の層）。関連: `AppContainer` の公開プロパティが死んでいても**クラス本体は生きていることがある**（今回 `BeanProfileMatchUseCase` は `SuggestUnexploredBeansUseCase` が自前に合成して現役だった）
- **発生源**: フェーズ 12-B の `BeanProfile` 導入時に作られ、2026-07-22 の産地ドロップダウン化で消費先を失った 4 シンボル。削除は 2026-07-31、commit `095b013`
- **横展開点検（2026-07-31）**: ①ブリッジ層の全数点検 — `grep -n "^\(suspend \)\?fun AppContainer\." shared/framework/.../AppContainerViewModelFactory.kt` で拡張 9 個を列挙し、各名で `iosApp` / `androidApp` を grep → `fetchBeanSuggestions` 以外の **8 個すべてに呼び出し元あり**。`shared/framework` の残り 1 ファイルは `internal object FrameworkMarker`（空モジュール対策）で公開面ゼロ。②Swift 向け公開 API の全数点検 — `grep -rn -A3 "@Throws" shared` で `@Throws` 付き関数を抽出し、interface 宣言 18 個について宣言・override 行を除いた呼び出し元を計数 → **全件 1 件以上**（最小は `summarizeBeanTraits` の 2 件で、`AnalysisViewModel.kt:387` に実呼び出しを確認）。**要削除は今回の 4 シンボルのみで対処済み**。判定手順は `.claude/rules/kotlin-kmp.md` の「機能撤去時の点検経路」へ昇格

## 2026-08-01

### doc の「更新するか否か」を、その doc を開かずに判断すると、対象範囲を自分の思い込みで狭めてしまう

- **症状**: 写真の保存時リサイズを計画する際、`paid-services.md` を「写真はローカル完結で課金対象サービスを使っていないから更新不要」とプランに明記した。ユーザーの「paid-services は更新必要では？」で開いたところ、**`Cloud Storage for Firebase` の行が既に存在**し、doc 冒頭は対象を「課金が発生する**または将来発生しうる**もの」と定義していた。さらに更新ルールが「Storage 復活」を明示的にトリガーに挙げていた
- **原因の構造**: 判断に使ったのは doc の中身ではなく、**doc 名から連想した守備範囲**（「有料サービス棚卸し」＝ 今課金されているものの一覧だろう）。この推測は半分合っていて、`Places` / `Firestore` は実際に課金中の行なので、**部分的に正しい思い込みほど検証されずに通る**。加えて今回は「写真をクラウドに置くかは今回の判断対象外」と切り分けた直後で、**スコープを絞る判断の勢いが、doc の対象範囲を絞る判断にそのまま流れ込んだ**。実際には切り分けた側（将来のクラウド保持）こそがこの doc の管轄だった。CLAUDE.md は親に「doc に書く事実は、書く前にソースを開いて確かめる」を課しているが、**「書かない」という決定にも同じ検証が要る**ことが明文化されていなかった（不作為は差分に現れないので、レビューでも捕まらない）
- **修正パターン**: 変更に着手する前に、CLAUDE.md 必読 doc 表を**上から 1 本ずつ**、その doc の前文・更新ルールと今回の変更を突き合わせる。「関係なさそう」で飛ばさず、**関係ないと判断した doc も開いた上で判断する**。今回この手順をやり直したところ、見落としは `paid-services` だけでなく `ui-ux-guidelines` にもあった（下記点検）
- **教訓**: **未採用・未実装のものにも行を持つ doc がある**（`paid-services` の `Cloud Storage` / `Cloud Functions`、`requirements` の未決事項）。この種の doc では「今その機能を使っていない」は更新不要の理由にならず、むしろ**将来のコストや判断を決める変数を変えたときこそ更新対象**になる。判断の重心は「今どうなっているか」ではなく「**この doc は何を管轄すると自分で宣言しているか**」に置く
- **発生源**: 写真の保存時リサイズ（2026-08-01、`ImageDownsampler` 導入）。プラン `.claude/plans/streamed-humming-lighthouse.md` に「`paid-services.md` は更新しない」と明記していた。ユーザー指摘で是正。CLAUDE.md「doc に書く事実は…」の項へ不作為分を追記して昇格済み
- **横展開点検（2026-08-01）**: 今回の変更について CLAUDE.md 必読 doc 表を全件突き合わせた。**ヒット 1 件** — `ui-ux-guidelines.md:286` の写真対比表「表示枚数 = 記録写真: **制限なし（全件）**」が、10 枚上限の導入で誤読を招く（View が絞らないことと添付上限が無いことは別）→ 「添付されている全件を表示する。ただし添付そのものに上限がある」へ是正 + 取得元セルに保存時リサイズを追記（値は複製せず requirements 参照）。**該当なし**: `app-store-metadata.md:166` 写真の申告は「端末内保存・サーバ送信なし」のみで解像度・枚数に非依存（開いて確認）/ `coding-conventions.md:325`・`architecture.md:60` の iosApp ツリーは `Utilities/ … 等` `ほか` 表記で、既存ディレクトリへの 1 ファイル追加では陳腐化しない（開いて確認）/ `kmp-bridge.md` は KMP 公開 API 変更なしのため非該当

### 文言の一括変更を `--include="*.swift"` で sweep すると、最も多くのユーザーが読むコピー（Info.plist）を取りこぼす

- **症状**: 旧用語「訪問記録」→「コーヒー記録」の統一で、親が `grep -rn "訪問記録" iosApp --include="*.swift"` を実行し「該当 4 箇所」と確定して dispatch した。実際は **5 箇所**で、5 件目は `iosApp/iosApp/Info.plist` の `NSPhotoLibraryUsageDescription`（写真ライブラリ許可ダイアログの説明文）。サブエージェントが `--include` 無しで再実行して発見した
- **原因の構造**: `--include="*.swift"` は「iOS のコピーは Swift にある」という無意識の前提。だが**アプリ内で最も多くのユーザーが読む文言は Swift の外にある** — `Info.plist` の `NS*UsageDescription` は OS の許可ダイアログとして**全ユーザーに 100% 表示**され、さらに App Store 審査の確認対象でもある。アプリ内の設定画面の説明文（開かない人が大半）より露出が桁違いに大きいのに、grep の対象からだけ外れる。加えて `.swift` に絞ると**ノイズが減って結果が読みやすくなる**ので、絞ったことの代償に気づきにくい
- **修正パターン**: **ユーザー向け文言の sweep では拡張子を絞らない**。絞るなら先に「文言が存在しうるファイル種別」を列挙してからにする: `Info.plist`（`NS*UsageDescription` / `CFBundleDisplayName`）/ `*.strings` / `*.xcstrings` / `*.stringsdict` / `*.entitlements`。逆に**コード上の概念名**を探すときは `--include` を付けてよい（用途が違う）
- **教訓**: **grep の `--include` は「探す対象」ではなく「読みやすさ」で付けてしまう**。文言・用語のような**プロダクト側の一貫性**を点検するときは、まず全文検索して件数を見てから絞る。今回は同じ場所で **`NSPhotoLibraryUsageDescription` だけ末尾の句点が無い**という別の不揃いも同時に見つかっており（他 2 つの `UsageDescription` は「。」で終わる）、これも Swift だけ見ていた限り永久に見つからなかった
- **発生源**: ASO-6 ①② のコピー実装（2026-08-01）。旧用語は フェーズ 7 の `Visit` → `CoffeeRecord` 転換（2026-06-19）の残骸で、**1 年半近く OS の許可ダイアログに出続けていた**
- **横展開点検（2026-08-01）**: ①拡張子を絞らない全文検索 `grep -rn "訪問記録" iosApp shared androidApp` → **0 件**（Swift 4 + plist 1 をすべて是正済み）②「訪問」を含む全ヒットを目視 → 残りはすべて **「訪問済みカフェ」** で、`VisitedCafe`（data-model §1.5）は現行の生きた概念のため対象外（旧用語は「訪問記録」だけ）③文言を持ちうるファイル種別の棚卸し → `.strings` / `.xcstrings` / `.stringsdict` は**リポジトリに 0 件**（ローカライズカタログ未整備。ASO の英語化で新設したら sweep 対象に加わる）、`Info.plist` のユーザー可視文字列は `CFBundleDisplayName` + `UsageDescription` 3 件で全件確認済み ④`shared/**` の日本語文字列 → `DummyCoffeeData.kt` のみで、`#if DEBUG` 相当の開発用データのため製品コピーではない

### doc / KDoc に書いた「数え上げ」は、要素が 1 つ増えるたびに全項目がまとめて嘘になる

- **症状**: `implementation_note.md` の「現在生きてる方針サマリ」（この doc で最初に読まれる節）に、`AppContainer` の「プライマリ 7 引数 / iOS 6 / Android 5」（実際は **9 / 8 / 7**）と、削除済みシンボル `beanProfileMatchUseCase` を含む公開プロパティ列挙、`curatedCafes` を欠いた Firestore コレクション列挙が残っていた。**行数は閾値未満（1062 < 1200）でアーカイブ不要**だったので、行数だけ見ていれば発見されなかった
- **原因の構造**: 数え上げ・構成要素の列挙は、**要素が 1 つ増えるたびに個数と一覧の両方が同時に古くなる**。しかも増やした本人はソース（`AppContainer.kt` / `settings.gradle.kts` / `firestore.rules`）しか見ないので、別 doc の列挙が壊れたことに気づく契機がない。**壊れ方が「静か」なのが本質**で、コンパイラもテストも grep も鳴らず、次に doc を読んだ人が実装と違うことに気づくまで残る。今回の 3 件はいずれも「`savedCafeRepository` / `curatedCafeRepository` を足した」という**正しい実装変更の副作用**で、誰も間違ったことをしていない
- **修正パターン**: **「どこを真とするか」を書き、数えない**。`AppContainer.kt` / `settings.gradle.kts` / `firestore.rules` のように**列挙の正本になるファイルは必ず存在する**ので、doc にはポインタと、数えなくても変わる構造・不変条件（「プライマリはテスト専用」「iOS だけ provider を注入する」等）だけを書く。**同じ行で「正確な一覧は X を真とする」と書きながら列挙も併記しない**（今回 feature モジュール行がこの形だった。読み手はどちらを信じるべきか分からない）
- **教訓**: これで **3 例目**（`CuratedCafe.kt` KDoc のピン種別 2026-07-25 / `SavedCafePin` KDoc の「既存 3 種ピン」2026-07-27 / 本件 2026-08-01）。KDoc でも doc でも同じ形で起きるので、**「N 個」「N 種」「A / B / C の 3 つ」と書きかけたら、それが増えうるものかを一度止まって考える**。増えうるなら書かない。実装ノートの前文にルールとして明文化済み
- **発生源**: `docs/implementation_note.md`「現在生きてる方針サマリ」。2026-07-25 の棚卸し以降、`savedCafeRepository`（10-x 行きたい店）/ `curatedCafeRepository`（フェーズ 19 おすすめ）/ `exportCoffeeRecordsUseCase` の追加と `beanProfileMatchUseCase` の削除（2026-07-31）が反映されていなかった
- **横展開点検（2026-08-01）**: `grep -rnE "[0-9]+ ?(個|種|引数)" docs/*.md` で全 doc の数え上げを抽出し、**日付付きエントリ内の史実記述**（「当時 3 種だった」等・更新不要）と**現在形の主張**を仕分けた。現在形は 2 件のみ: ①`implementation_note.md` の feature モジュール「8 個」= 数は正しい（`settings.gradle.kts` と一致）が列挙併記が上記アンチパターンのため削除 ②`app-store-metadata.md` の「収集データ 3 種（Crash / Performance / ProductInteraction）」= `PrivacyInfo.xcprivacy` を `plutil` で実読みし `NSPrivacyTracking = false` 込みで**一致を確認**（据え置き）。**要修正は ① のみで対処済み**。あわせて `curate-doc` skill のフロー型ルーティングに「present-tense の節にだけ Phase 1」を追加（行数が閾値未満でも present-tense 節の照合だけは通す運用に変更）
- **横展開点検 その 2 — コード側（2026-08-01、同日追加）**: 上の sweep は `docs/*.md` しか見ておらず**KDoc を見落としていた**（同じ「対象範囲を絞りすぎる」失敗を同じ日にやっている）。`grep -rnE "^\s*\*.*[0-9]+ ?(個|種|引数|箇所)" shared --include="*.kt"` と `grep -rn "init(sqlDriver" shared --include="*.kt"` で追加点検し、**4 件検出・全件是正**（tasks B-9 に合流）。内訳: ①`AppContainer` クラス KDoc の Swift init シグネチャ書き写し 2 件が古く、**同ファイルのセカンダリコンストラクタ KDoc と矛盾**（起票の直接原因）②同 `:115`「上の 8 引数セカンダリコンストラクタ」（数は正しいが同じ形）③`AnalysisViewModel` が**同一ファイル内で「カテゴリ 3 種」と「カテゴリ 4 種」を併記**（2026-07-20 の精製方法軸追加に片方だけ追随。実態は 4）④`MapViewModel` の「第 4 種ピン」「4 種目ピン」という**序数指定**（当時 4 種 → 現在 6 種で何も指していない。ついでに「検索タブ」= 廃止済みも是正）。いずれも数を消して意味で書く形に統一した
- **副産物 — 固定データセットの内訳コメントもズレる**: 上の sweep で残った `DummyCoffeeData.kt` を「そのファイルを編集しない限り変わらないから許容」と判断しかけたが、念のため実データと突き合わせたところ **3 箇所の実数不一致**が出た（「その他 18 件」→ 実際 20 / 「産地 8 種」→ 9 / 「焙煎度 null 3 件」→ 4）。2026-07-20 の人格再設計でデータ側を作り替えた際、内訳コメントが部分的にしか追随していなかった。**「変わらないはずのもの」も、実際には変わっている**ので、除外理由が「変わらないから」のときこそ一度数える
- **横展開点検 その 3 — Swift とエージェントメモリ（2026-08-11、docs 棚卸しで再発）**: **4 例目**。`Cafe` の永続フィールドを「8 フィールド」と書いた記述が **4 箇所**（`SavedCafe.kt` の `@property` / `SavedCafeFirestoreMapper.kt` / `CoffeeFirestoreMapper.swift` ×2 / `SavedCafeFirestoreMapper.swift`）残っていた。2026-08-07 に `photoAttributions` を永続対象化した際の追随漏れで、**マッパ実装は 3 経路とも 9 フィールドを正しく扱っており、壊れていたのはコメントだけ**。その 2 の sweep が `shared --include="*.kt"` に絞っていて **`iosApp/**` の Swift を見ていなかった**のが見落としの直接原因（**同じ「対象範囲を絞りすぎる」失敗の 3 回目**）。今回の sweep 範囲は `shared/ iosApp/` × `*.kt` `*.swift` に加え **`.claude/agent-memory/**`** まで広げ、`ios-engineer` のメモリにも同型 2 件（うち 1 件は「`Cafe` は Swift 側で 14 引数の init 1 本」= 現在 15 で**実数として壊れていた**）を発見・是正した。**エージェントの永続メモリは git 管理下の doc と同じ陳腐化をするのに、これまで一度も sweep 対象に入っていなかった**
- **判定軸の精緻化 — 危ないのは「加算的に増える集合」の数え上げ（2026-08-11）**: 今回の sweep は 40 件超の「N 要素 / N 軸 / N フィールド」をヒットさせたが、**是正が要ったのは `Cafe` 系だけ**だった。残り（`TastingScores` の「5 要素」/ `PreferenceMatchAxis` の「4 軸」）は**閉じた集合**で、増やせば全構築箇所がコンパイルエラーになるため静かには壊れない。壊れるのは **`Cafe` のようにデフォルト値付きで末尾に足せる = コンパイラが何も言わない集合**。「N と書くな」ではなく **「コンパイラが増加を検出しない集合を数えるな」** が正しい線引きで、この判別ができると sweep のノイズが一気に減る。合わせて `data-model.md` §1.2 に「**このフィールド列挙が正本。他の節・コードコメントは件数を書かずここを参照する**」と明記し、doc 内に 6 箇所あった「9 フィールド」も参照表現に置換した（doc 自身が同じ地雷を 6 個持っていた）

### `.onChange` から起こす `Task { }` は非構造化で、ビューが消えてもキャンセルされない

- **症状**: ASO-1 のレビュー依頼で「1.5 秒待ってからダイアログを出す」実装をしたところ、**遅延中に分析タブを離れると他タブの上にダイアログが出る**経路が残った。`.task` 経由の呼び出しはキャンセルできるのに、`.onChange` 経由だけができない、という非対称が生まれていた
- **原因の構造**: `.onChange(of:) { Task { ... } }` の `Task { }` は**非構造化タスク**で、ビューのライフサイクルに紐づかない。ビューが消えても走り続ける。一方 `.task` はビュー消滅でキャンセルされる。**同じ処理を「初期状態」と「変化」の 2 経路で呼ぼうとすると、この 2 つを並べる書き方に自然に行き着く**（`.task` で初期状態、`.onChange` で遷移）ため、非対称は意図せず生まれる。さらに `try? await Task.sleep(...)` は**キャンセル時に nil を返すだけで実行が次行へ進む**ので、キャンセルされても処理が完走してしまう（`try?` が握り潰す）
- **修正パターン**: **`.task(id:)` 1 本に統合する**。`.task(id:)` は ①表示時に 1 回起動 ②`id` 変化のたびに前タスクをキャンセルして再起動 ③ビュー消滅時にキャンセル、をまとめて満たすので、`.task` + `.onChange` の 2 経路が持っていたカバレッジを包含する。`id` は `Equatable` であればよい（`Bool?` / `[PhotosPickerItem]` とも可）。遅延を挟むなら `try? await Task.sleep(...)` の直後に **`guard !Task.isCancelled else { return }`** を置く。**副作用を確定させるフラグ更新は、このガードより後**に書く（キャンセル回を「実行済み」に数えないため）
...
### 「成果物で検証する」は、**どの成果物が提出物か**まで詰めないと、修正と無関係な値を観測して誤った結論に至る

- **症状**: アプリアイコンのアルファチャンネル除去（Apple の要件対応）にあたり、親が `ios-engineer` へ **「検証はビルド成果物側で行う。`sips -g hasAlpha <app>/AppIcon60x60@2x.png` が `no` になること。**ここが本丸**」**と検証手段まで指定して dispatch した。サブエージェントが実測したところ、この派生 PNG は **Debug ビルドでは新旧どちらのソースでも `hasAlpha: yes`**、**Release / Archive では新旧どちらでも `no`** で、**ソースの修正と完全に無関係**な値だった。親が指定した検証手段そのものが成立していなかった
- **原因の構造**: `actool` は configuration によってアイコン派生 PNG のアルファ扱いを変える（Debug は RGBA コンテナのまま、Release は `--compress-pngs` 経路でアルファ面を落とす）。**「ビルド成果物を見る」という原則自体は正しい**のに、`.app` の中には**提出物ではない中間生成物**が混ざっており、そこを見ると configuration の副作用を観測してしまう。ASC が実際に読む 1024 マーケティングアイコンは `Assets.car` の中にしかなく（アーカイブに `ProductIcon.png` は存在しない）、`xcrun assetutil --info` の `Opaque` フラグで見るのが正しい対象だった。**失敗の入り口は同日の成功体験**で、数時間前に `developmentRegion` を「ビルド成果物の `Info.plist` を実読みして確認する」で正しく検証できたため、その型を**対象の性質を確かめずに別の問題へ転用した**
- **修正パターン**: 成果物検証を設計するときは「**この値は、私の変更以外の要因で変わりうるか**」を先に自問する。答えが「変わりうる」なら検証手段として使えない。確実な切り分けは**変更前後で同じ手順を踏んで差が出ることを確かめる**こと（今回サブエージェントは旧ソースでアーカイブし直して「差が出ない」と示した。これが正しい潰し方）。アイコンについては Release / Archive + `assetutil --info` の `Opaque` を見る。**ローカルの `-validate-for-store` はアルファをチェックしないので、ASC 通過の証明にはならない**
- **教訓**: **サブエージェントに検証手段まで指定するときは、その手段が仮説に依存していることを自覚して「成立しなければそう報告してよい」と明示する**。今回は「**ここが本丸**」と強い言い方で渡したため、実測が食い違ったときにサブエージェント側が判断に迷う余地を作った（結果的に正しく否定して返してきたのは良い仕事）。親の仮説は**指示ではなく仮説として**渡す
- **発生源**: 2026-08-06、アプリアイコンのアルファ除去。**同日 2 件目の検証手段ミス**（1 件目は `plutil -extract` の `-o` 省略でファイルを破壊）。どちらも「検証しているつもりで、観測対象が壊れている / 無関係」という同じ形
- **横展開点検（2026-08-06）**: 「configuration や環境で値が変わる中間生成物を、検証の根拠にしていないか」の観点で docs 内の検証手順を点検 → `implementation_note.md:939`（`UIDeviceFamily` を成果物 `Info.plist` で確認）は **`GENERATE_INFOPLIST_FILE` 由来の最終値で configuration 非依存のため妥当**（据え置き）。`app-store-metadata.md` §10 の「App Icon が全サイズ揃っている」は資産カタログを直接見る手順で妥当。**要是正は今回追加したアルファ項目のみ**で、Debug 検証が無効である旨を明記して対処した

### `List` / `ScrollView` の中の `DragGesture` は、①スクロール奪取 ②touch down の副作用 ③キャンセル時の固着 を **3 点セット**で潰す

- **症状**: 標準 `Slider` がトラックのタップを無視する（つまみを正確に掴まないと動かない）ため、`Form` 内のテイスティングスライダーを自前ジェスチャー駆動の tap-to-seek に置き換えたところ、**修正のたびに次の副作用が出る**という連鎖になった。①`.gesture(DragGesture(minimumDistance: 0))` → その行の上から始めた縦スワイプで Form がスクロールできない ②`.simultaneousGesture` に変更 → スクロールしようと**指を置いただけでテイスティングの値がその x 位置に書き換わる**（保存データが黙って変わる）③縦優勢と判定したら開始値へロールバックするラッチを追加 → **`onEnded` が呼ばれない経路でラッチが解除されず、そのスライダーが以後まったく反応しなくなる**
- **原因の構造**: 3 つとも別々のバグではなく、**スクロールコンテナ内でジェスチャーを共存させることの表と裏**。(a) `.gesture` は親のスクロール用パンより優先される (b) `DragGesture(minimumDistance: 0)` は **touch down の瞬間に `onChanged` が発火する**ので、「タップで即反応」と「触れただけで反応してしまう」が原理的に同じ挙動 (c) SwiftUI のジェスチャーは他ジェスチャーに競り負けて**キャンセル**されると `onEnded` を呼ばない。①を直すと ② が出て、② を直すと ③ が出る構造なので、**着手時に 3 点まとめて設計しないと必ず往復する**
- **修正パターン**: ①スクロールと共存させるなら `.simultaneousGesture`（`.gesture` は奪う）②touch down 発火を前提に、ジェスチャー開始時の値を保持し「縦方向の移動が支配的（`|height| > 閾値` かつ `|height| > |width|`）」になったらスクロール意図と判定して**開始値へロールバック**する ③**状態のリセットを `onEnded` に置かない**。`onChanged` の中で「これは新しいジェスチャーの最初のイベントか」（`translation` が 0 近傍）を毎回判定し、そこで状態をまとめて初期化する。`onEnded` は保険として残す。**終端イベントに依存する状態機械は、キャンセルされうる文脈では必ず固着する**
- **教訓**: **タップ位置に即反応させる UI は「意図しない接触にも即反応する UI」と同義**。スクロールできる画面に置く時点で、意図の判別（方向・距離）とロールバック経路がセットで要る。加えて **`minimumDistance: 0` を書いたら、キャンセル経路での状態復元をその場で確認する**。ジェスチャーの状態を `@State` に持つときは「開始で初期化・終了で使う」であって「終了で初期化」ではない
- **発生源**: 2026-08-06、`TappableTastingSlider`（`CoffeeEditorView+Sections.swift`）。ユーザー報告「つまみじゃないところをタップしても変わるようにしたい」への対応。②③はいずれも**サブエージェントのビルド成功報告後に親のレビューで検出**したもので、ビルドは 3 案とも通っていた（`Verification Before Done`: ジェスチャー競合はコンパイラが一切守ってくれない領域）。設計判断の詳細は implementation_note 2026-08-06
- **横展開点検（2026-08-06）**: `iosApp` を `DragGesture` / `simultaneousGesture` / `highPriorityGesture` / `minimumDistance` で grep → 実装は 3 箇所。**該当なし**: `ErrorToast` は `minimumDistance: 20` + `onEnded` のみで状態を持たない（②③とも非該当）。`MapSearchResultsSheet.handleBar` は `minimumDistance: 8`（②非該当）+ ドラッグをハンドル行のみに限定して下部 `ScrollView` と競合させない設計（①非該当）。③は `onEnded` で `dragTranslation = 0` に戻す形のため理論上はキャンセルで残りうるが、**次のドラッグの `onChanged` が上書きして自己回復する**（更新を止めるラッチではない）ので据え置き。**固着するのは「以後の更新を止める」種類の状態だけ**、が判別の目安

### 永続ストアを 1 つ増やしたら、**削除経路**にも 1 行足す — 消し漏れはコードのどこにも現れない

- **症状**: アカウント削除の実機検証で、削除後の Firestore に `users/{uid}/savedCafes` と `users/{uid}` ルートドキュメントが**そのまま残っていた**（消えていたのは `coffees` だけ）。`DeleteAccountUseCase` は「coffees をループ削除 → `deleteAuthUser()`」の 2 段しか持たず、`users/{uid}` 配下 3 要素のうち 2 つに一度も触れていなかった。`savedCafes` はフェーズ 15-A で追加されたが、**そのとき削除経路への追随が行われなかった**
- **原因の構造**: **消し忘れたコレクションは、コードのどこにも現れない**。フィールド追加の漏れ（07-08 / 07-25 のマッパー追随漏れ）はまだ「そのフィールドを書いた場所」が grep できるが、削除経路の漏れは**書かなかったことによるバグ**なので、型検査もコンパイラも既存テストも一切検出しない。実際この時点のテストは「coffees が消えてから `deleteAuthUser` が呼ばれること」を検証しており、**書いた分については正しく green だった**。加えて発見が難しい: アプリ内では削除後に新しい匿名 uid へ切り替わるため、旧 uid のデータは**画面上どこにも出てこない**（クエリが uid で絞る）。今回も実装作業ではなく **revoke の実機検証中に Firestore コンソールを開いていて**気づいている
- **性質が悪いのは「消し残り」で済まないこと**: Security Rules が `request.auth.uid == uid` を要求するため、**Auth ユーザーを消した瞬間に残留データは本人が二度と読めず消せない永久孤児になる**。運営側からのみ見える状態で、プライバシーポリシーの「アカウント削除により削除されます」と真っ向から矛盾する。**削除処理は「成功」を返しているのに、取り返しがつかない**
- **修正パターン**: 削除は**サブコレクション → 親ドキュメント → 認証ユーザー**の順に段を積む。順序は好みではなく仕様で、根拠が 2 つある。(a) **Firestore はドキュメントを削除してもサブコレクションをカスケードしない**（親を先に消すと子が孤児化）(b) **Auth ユーザー削除後は Rules で配下に一切到達できない**（Auth を先に消すと全部が孤児化）。テストは各段の**早期中断**を段数ぶん書く — 特に「親ドキュメント削除が失敗したら認証ユーザーを消さない」を明示的に検証する（ここが通っていないと、失敗時に永久孤児を作る経路が残る）
- **教訓**: **ユーザーに紐づく永続ストア（Firestore コレクション / DB テーブル / ファイル領域）を新設したら、その変更と同じ差分で削除経路にも追随する**。`.claude/rules/kotlin-kmp.md` には「フィールドを足したら写る先 6 経路」のチェックリストがあるが、これは**フィールド**の話で、**ストアそのものを増やしたとき**は別軸。削除範囲の正本を doc 側に置き（data-model §3.1「アカウント削除で消す範囲」）、そこと実装の 2 箇所を同時に更新する運用にした
- **発生源**: 2026-08-06、`DeleteAccountUseCase`。原因を作ったのはフェーズ 15-A（`savedCafes` 追加）で、**約 3 週間気づかれなかった**。設計判断と「coffees を分析用に残す案」の不採用理由は implementation_note 2026-08-06
- **横展開点検（2026-08-06）— ユーザー紐付きの永続ストアを全数え上げ**: **該当なし**（今回の修正で全て被覆）。内訳は次のとおり。
  - **Firestore**: `grep -o 'collection("[^"]*")'` を `iosApp` + `shared/data-firebase` に適用 → `users` / `coffees` / `savedCafes` / `beanProfiles` / `curatedCafes` の 5 つのみ。**uid でキーされるのは `users/{uid}` 配下だけ**で、`beanProfiles` / `curatedCafes` はサービス管理のグローバルコレクション（クライアント read-only）なので削除対象外。`sharedTasteProfiles`（9-6）は未実装
  - **SQLDelight**: テーブルは `coffee_record` / `photo` / `saved_cafe` の 3 つ。`coffee_record` と `saved_cafe` は 1 件ずつの `delete` がローカル行も消す（`CoffeeRepositoryImpl` / `SavedCafeRepositoryImpl` が local + remote の合成）。**`photo` は `ON DELETE CASCADE` 依存なので FK が有効かを両ターゲットで実確認した** — Android は `setForeignKeyConstraintsEnabled(true)`、iOS は `NativeSqliteDriver(onConfiguration = ... foreignKeyConstraints = true)` で**両方とも有効**（06-19 / 07-03 の FK 教訓の対応が効いている）。ここが片方でも OFF だと同型の孤児行が出る
  - **ファイル**: `PhotoFileStore.deleteAllPhotos()` が削除完了後に走る（iOS 責務）
  - **UserDefaults**: `adConsentFlowShownKey` / `ApplePoiNegativeCache` / ReviewPrompt の `milestoneShownKey` の 3 つ。いずれも**端末スコープの設計で意図的にアカウント削除の対象外**（ATT は端末単位、レビュー依頼の 1 回きりも端末単位が Apple の想定、POI ネガティブキャッシュは uid 非依存の技術キャッシュ）。ユーザー生成データではないため削除不要と判断

---

## 2026-08-07

### 外部 API の**表示義務・帰属義務**は、動作にもコンパイラにもテストにも現れない — 採用時に課金と一緒に doc 化する

- **症状**: App Store Review Guidelines に沿った全体レビューで、**Google の帰属表示がアプリ内に 1 つも無い**ことが判明（`grep` で 0 件）。Places API の店名・住所・評価・営業時間・写真を Apple MapKit + 自前 UI 上に表示しているが、Places のポリシーは (a) Google マップ以外に表示する場合の **Google Maps ロゴ（狭ければ「Google Maps」テキスト）** (b) **写真表示時の作者クレジット**を要求している。(b) に至っては `Cafe` が `photoReferences` しか持たず、**帰属情報をモデルに載せていなかったので表示しようがなかった**（FieldMask は `places.photos` を丸ごと要求済みでレスポンスには最初から含まれていた）
- **原因の構造**: **規約違反は「書かなかったこと」で起きるので、あらゆる自動検査をすり抜ける**。帰属を出さなくても API は正常に応答し、アプリは完全に動く — むしろ**動いてしまうことが発見を遅らせる**。削除経路の漏れ（08-06）と同じ「書かなかったバグ」の系統だが、こちらは**そもそも要件として認識されていなかった**点がより悪い。決定的だったのは、`paid-services.md` §2b が「Places 由来のデータを広告ターゲティングシグナルに渡さない」という**別の規約項目は明記していた**こと。**規約の一部だけを doc 化すると、書かれなかった残りは「存在しない要件」になる**
- **修正パターン**: ①要件 doc（requirements §5）の冒頭に帰属義務そのものを注記し、**「Places データを新しい画面に出すときは帰属表示をセットで実装する」**という運用ルールにする ②帰属表示は共通コンポーネント 1 つ（`GoogleMapsAttributionText`）に閉じ、使用箇所と**出さない箇所の判断理由**をコンポーネント側の KDoc に書く（記録一覧・共有カードは保存済みスナップショット = ユーザー自身の記録の表示なので対象外、という線引き）③写真の作者帰属のように**モデルに載っていないと表示できない**ものは、永続化するか毎回取り直すかの設計判断になる。写真参照を永続化している以上、帰属も永続化しないと DB スナップショット由来の表示が無帰属になる
- **教訓**: **外部 API を採用するときは、課金と技術仕様だけでなく「表示義務・帰属義務・キャッシュ制限」を同じタイミングで doc 化する**。「動いているから規約を守れている」は成立しない。リスクの重みも直感と逆で、**App Store 審査で弾かれる確率より Google 側の是正要求 → API キー停止のほうが重い**（キーが止まればアプリの中核機能が全滅する）
- **発生源**: 2026-08-07、ユーザー依頼の App Store Review Guidelines 全体レビュー。Places API はプロジェクト初期から使っており、**約 2 か月間どのレビューでも指摘されなかった**。ガイドラインという外部の観点を通したときに初めて出た（実装レビューでは構造上出ない種類の欠落）。関連: `CoffeeRecord` への `Cafe` スナップショット永続保存自体もポリシー上グレー（無期限保存が明示的に許されるのは place ID のみ）だが、記録アプリの成立要件なのでリスク認識に留めた
- **横展開点検（2026-08-07）— 利用中の外部サービスの表示・帰属義務を全数え上げ**: **要是正は Google 系のみで、今回すべて対処済み**。内訳は次のとおり。
  - **Google Places API**: 上記 2 点とも未対応だった → 4 面に `GoogleMapsAttributionText`、写真は `CafePhotoHeader` に作者バッジを追加。小サムネイル（検索結果行 / ピン選択カード）はポリシーの「スペースが限られる場合、より大きな版で完全な帰属にアクセスできれば省略可」に依拠して省略（タップ → カフェ詳細で作者名に到達できることが根拠）
  - **ライセンス表示**: `LicensesView` に **Google Mobile Ads SDK / UMP SDK / Google Places API の 3 件が欠落**していた → 追加。**GoogleMobileAds を `Apache 2.0` と書かないこと** — SPM パッケージの `Package.swift` は Apache 2.0 だが、それはバイナリを取得するだけのラッパー層で、実体の `.xcframework` は AdMob 利用規約（独自条項）
  - **AdMob（広告の帰属 / AdChoices）**: バナーでは Google のレンダリングに内蔵されるため自前実装不要（requirements §11 の確定仕様に記載済み）。**該当なし**
  - **Apple MapKit / Foundation Models / Firebase**: ネイティブアプリでの利用に帰属表示の義務なし。**該当なし**

### `Cafe` のような**埋め込みスナップショット型**へのフィールド追加は 2 テーブルに波及し、別テーブルに触れる migration は「生 DDL で状態を再現する既存テスト」を壊す

- **症状**: `Cafe.photoAttributions` の追加で 3 つの追随漏れが連鎖した。①SQLDelight は `coffee_record` **と** `saved_cafe` の両方に列が要る ②migration 7 が `saved_cafe` にも `ALTER TABLE` を打った結果、**既存の `CoffeeRecordMigration5Test` / 同 IosTest が `no such table: saved_cafe` で失敗**した ③`scripts/seed/seed-coffees.mjs` の `cafe` 構築が明示 allowlist なので、追加しないと export JSON をインポートした瞬間に**無言で欠落**する
- **原因の構造**: ①は `Cafe` が**独立テーブルを持たず 2 つの親テーブルに埋め込まれている**ため。`.claude/rules/kotlin-kmp.md` の「フィールドを足したら写る先 6 経路」は `CoffeeRecord` のフィールドを想定した動線で、**`Cafe` のフィールドは SQLDelight の段だけで 2 箇所に増える**。②が構造的に面白い点: これらのテストは migration 4 相当の状態を**生 DDL で再現**しているが、`saved_cafe`（実際には 3.sqm で既に存在する）を作っていなかった。それでも今日まで green だった — **後から入る migration がそのテーブルに触れるまでは、再現が不完全でも誰も困らない**。つまり**生 DDL による状態再現は、将来の migration が別テーブルに触れた瞬間に嘘になる**（時限式に壊れる）。③は `dropNullKeys` が `v != null` 判定のため **空配列がフィルタをすり抜ける**。「空ならキー省略」を実装するには `undefined` に潰す必要があり、`?? []` を書き足す 1 行では規則を破る
- **修正パターン**: `Cafe` にフィールドを足すときは **SQLDelight 2 テーブル + migration（`ALTER TABLE` × 2）+ Mapper の 4 経路 + 各 Repository の `upsert` 呼び出し**をセットで見る。既存の migration テストが生 DDL を持つなら、**その DDL に今回触れるテーブルが含まれているか**を先に確認する。Firestore の省略規則は **iOS mapper / Android mapper / seed スクリプトの 3 経路**で揃える（今回 `photoAttributions` は「空ならキー省略」だが `photoReferences` は「空でも常に書く」で**意図的に非対称**なので、3 経路とも同じ非対称性を持たせる必要がある）
- **教訓**: 07-25 の「ドメインのフィールドが写る先は 5 経路・export DTO が抜けやすい」と同根だが、**埋め込み型（`Cafe`）は経路の数が型ごとに違う**。「`CoffeeRecord` のときはこれで足りた」を流用しない。加えて、**テストが持つ生 DDL は本番スキーマの複製なので、複製であるがゆえに独立して腐る**
- **発生源**: 2026-08-07、`Cafe.photoAttributions` 追加（上記の帰属義務対応）。①③は事前に想定して dispatch 指示に含めたため漏れなかったが、②は実装中に初めて顕在化した
- **横展開点検（2026-08-07）**: ①**生 DDL でスキーマを再現しているテスト** — `grep -rln "CREATE TABLE" --include="*.kt" shared/ | grep -i test` → **`CoffeeRecordMigration5Test` / `CoffeeRecordMigration5IosTest` の 2 本のみ**で、両方とも今回の変更で `saved_cafe` を追加済み。**該当なし（対処済み）** ②**`dropNullKeys` 系の「空配列すり抜け」** — `scripts/` を `node_modules` 除外で grep → 定義は `seed-coffees.mjs:133` の **1 箇所のみ**、利用は同ファイル内 4 箇所。今回 `photoAttributions` に `undefined` 潰しを入れて対処済みで、他の利用箇所は配列を扱っていない。**該当なし（対処済み）**

### `tint` を省くと**無言で `accentColor` にフォールバック**する — 概念色を持つ UI では省略が「ブランド色の誤用」になる

- **症状**: カフェ詳細の未保存「保存する」ボタンが淡いグレー地＋茶ラベルで描画され、**無効なボタンに見える**とユーザー指摘。原因は `.buttonStyle(.bordered)` に `.tint` が無く、環境から `accentColor`（ブラウン）を継承していたこと。同じ画面の保存済み側は `.tint(.indigo)` を持っていた。横断点検でマップの「保存済み」チップ（`MapFilterChipRow.swift`）にも同型があり、**件数バッジと選択時のカプセル塗りが茶**になっていた
- **原因の構造**: `tint` は SwiftUI では**環境値**で、指定しなければ黙って `accentColor` が入る。`TagChip` も `var tint: Color = .accentColor` を既定値に持つ。つまり**書き忘れがコンパイルエラーにならないどころか、「一応それらしいブランド色」で描画されるので目視でも気づきにくい**。壊れ方が「明らかに変な色」ではなく「別の概念の色」なので、レビューでも見落とす。決定的なのは、**両者とも同じファイル / 同じ関数のすぐ隣に正しい例があった**こと（保存済み側は indigo、好み一致チップは `tint: .pink`）— 揃っていないことは差分を見ても分からず、**全 use を並べて初めて分かる**
- **修正パターン**: 概念色を持つ UI では **`tint` を必ず明示する**。省略は「既定でよい」という意思表示ではなく、単に情報が無い状態。コンポーネント側に既定値がある場合（`TagChip.tint`）、その既定は**中立な用途のためのもの**であって、意味付けされた概念が既定に乗ってはいけない。`TagChip` は KDoc に既にその趣旨を書いていたが、**書いてあっても呼び出し側は守らなかった**ので、doc だけでは防げない
- **教訓**: **「指定しなければ既定値」の API は、既定値が妥当に見えるときほど危険**。`accentColor` はブランド色なのでどこに出ても一応馴染んでしまい、誤用が「なんとなく正しい」見た目になる。概念に色を割り当てた時点で、**その概念を描く全箇所を列挙して grep で突き合わせる**のが唯一の検出手段（型もコンパイラもテストも助けてくれない）。同系統は 2026-07-27 の「白フチの有無を混ぜると意図しない序列が生まれる」— どちらも**個々には正しく見えるが、全 use を並べると不揃いが見える**タイプ
- **発生源**: 2026-08-07、UI/UX 敵対的レビューの UX-4。指摘は「グレーで無効に見える」という表層だったが、掘ると意匠の意図ではなく指定漏れだった。2 箇所目（チップ）は**ユーザーが「しおりアイコンの色が茶色だけどこれはいいの？」と気づいて発覚**しており、親の横断点検が UX-4 の時点で保存概念の全 use に及んでいれば同時に取れていた
- **横展開点検（2026-08-07）**: ①**`TagChip` の全呼び出し（production）** — `grep -rn "TagChip(" --include="*.swift" iosApp/iosApp | grep -v Components/TagChip.swift` → 4 箇所。訪問済み（tint 未指定 = `accentColor` が**正**）/ 好み一致 `.pink` / 保存済み **← 該当・修正済み** / タグフィルタ（中立チップなので既定で正）。②**`.buttonStyle(.bordered*)` の全箇所と `.tint` の有無** — 10 箇所。概念色を持つのは保存系 2 箇所のみで、`CafeSelectionCard`（indigo 済み）と `CafeDetailView`（**該当・修正済み**）。残り（共有 / 広告プレプロンプト / オンボーディング / エディタのサジェストチップ / エラートースト）はいずれも中立またはブランド色が正。③**`accentColor` の全使用箇所が概念色の文脈に紛れていないか** — 50 箇所超を目視。`SavedCafeListSheet:78`（「記録あり」）と `CafeSelectionCard:129`（「N杯」）は保存系の画面内にあるが**訪問済み概念**の表示なので `accentColor` が正しく、誤用ではない。**保存概念の全 6 箇所中 2 箇所が該当し、両方修正済み。他の概念（好み一致 pink / 検索結果 blue / おすすめ orange）に漏れなし**

### 続報: `List` / `Form` の中では **`tint` を明示しても `Label` のアイコンだけ効かない**

上のエントリ（`tint` 未指定 → 無言で `accentColor`）を修正した直後、**同じボタンで色がまだ揃っていない**とユーザーから再指摘。原因は別物だったので分けて記録する。

- **症状**: `CafeDetailView.saveButton` に `.tint(.indigo)` を入れた後も、**文字は indigo になったのに `bookmark` アイコンだけ茶（`accentColor`）のまま**。前エントリの修正は効いているのに見た目が直りきらない、という紛らわしい状態
- **原因の構造**: この保存ボタンは `cafeDetailList` の **`List` の中**にある。`List` / `Form` 内の `Label` は**アイコンだけ `tint` と `buttonStyle` を無視して `accentColor` で描かれる**。`.tint()` は塗り・枠線には効くのでボタンの外見は変わり、**「効いていないわけではない」ため原因の切り分けが難しい**。前エントリが「指定を書き忘れる」バグなのに対し、こちらは**正しく書いたのに一部だけ無視される**バグで、対処が根本的に違う（明示しても駄目なので、色の指定経路そのものを変えるしかない）
- **確認方法（再現手順として残す）**: `iOSApp.swift` の root scene を一時的にスクラッチ View へ差し替え → `xcodebuild` → `xcrun simctl install / launch / screenshot` で実画面を撮る。これで **①`List` 内 `bordered` + `tint` → アイコンだけ茶（症状再現） ②`List` 内 `borderedProminent` + `tint` → こちらもアイコンだけ茶 ③`List` の外 → 文字・アイコンとも揃う ④`Label` に `.foregroundStyle` を明示 → 揃う** を実測で確定させた。**「List 特有」を疑ったら List の外に出した対照を撮る**のが決め手
- **修正パターン**: `tint` からの継承に頼らず、**`Label` に `.foregroundStyle` を明示して文字とアイコンを強制的に揃える**。`.tint` は塗り・枠線用に残す。ラベルを分岐で使い回していると状態ごとの色を出し分けられないので、**分岐ごとに `Label` を構築し直す**（`let label` の共有をやめる）。副産物として、報告に無かった**保存済み状態（`borderedProminent`）でもアイコンが茶だった**ことが実測で判明し同時に修正した
- **教訓**: **「コンテナが子の見た目を書き換える」タイプの挙動は、子側のコードをいくら読んでも分からない**。色が揃わないときは、まず**同じ View を別コンテナに置いた対照実験**を作る。加えて、**ユーザー報告は不具合の全数ではない** — 報告されたのは未保存状態だけだったが、実測したら保存済み状態にも同じ不具合があった。報告を修正の範囲と見なさず、その原因が及ぶ範囲を自分で確定させること
- **発生源**: 2026-08-07、UX-4 の修正直後にユーザーが「しおりアイコンが茶色のまま」と再指摘
- **横展開点検（2026-08-07）**: `.tint(` の非 `accentColor` 明示箇所は `CafeDetailView.saveButton`（**該当・修正済み**）と `CafeSelectionCard`（マップ下部カードの保存ボタン）の 2 箇所のみ。後者は **`List` の外**（`.safeAreaInset` オーバーレイ）にあり、かつ `Label` ではなく `Image(systemName:)` 単体なので**該当なし**。`MapFilterChipRow` の `TagChip` は自前で `foregroundStyle` を当てる実装のため `List` 挙動の影響を受けない。その他の `buttonStyle(.bordered*)` は明示 `tint` を持たず文字・アイコンとも `accentColor` で揃っているため対象外

### sweep の grep を `head -N` で切ると、**点検が静かに不完全になる**

- **症状**: UX-3（追加ボタンを FAB → ナビバー右上へ統一）で、追随が要る doc を洗うのに `grep -rn "FAB\|フローティング\|右下" docs/*.md | head -10` を実行。ヒットが `app-store-metadata.md` と `implementation_note.md` で 10 行埋まり、**`requirements.md` の 4 箇所と `verification-checklist.md` の 3 箇所が出力に現れないまま**「更新対象は 2 本」と判断して commit した。4 タスク後（UX-7/8 の着手時）に requirements を別目的で開いて発覚
- **原因の構造**: `head` はエラーも警告も出さず、**「ヒットが少なかった」と「切られた」が出力上まったく区別できない**。しかも `grep -r` の走査順はほぼファイル名順なので、**アルファベット順で後ろのファイルほど落ちる**（`requirements.md` は `r` で後方）。探索的な grep で `head` を付けるのは正しい癖だが、**それをそのまま「網羅チェック」に流用した瞬間に嘘になる**。今回は「更新不要と判断した doc も開いて確かめる」という 2026-08-01 の教訓は守ったつもりでいたが、**そもそも候補リストが欠けていた**ので守りようがなかった
- **修正パターン**: **sweep の grep には `head` を付けない**。件数が多くて読みづらいなら、`head` ではなく ①`--include` やパスで対象を絞る ②`-l`（ファイル名のみ）で全体像を見てから個別に開く ③`| wc -l` で件数を確認してから出す、のいずれか。**「全部出す」ことが目的の grep と「雰囲気を掴む」grep を混ぜない**。前者では出力が長いことは欠点ではない
- **教訓**: **横断点検の信頼性は grep パターンの質だけでなく、出力を切っていないかにも依存する**。パターンが正しくても表示が切られていれば結果は同じく不完全で、しかも**成功したように見える**（0 件なら「該当なし」と気づけるが、部分ヒットは「これで全部だ」と誤認させる）。同系統は 2026-08-01 の「更新不要の判断も doc を開いてから」— どちらも**チェックしたつもりの範囲が実際より狭い**タイプ
- **発生源**: 2026-08-07、UX-3 の doc 追随。見落とした 7 箇所（requirements の機能表 2-1 / 2-8・画面一覧 2 行、verification-checklist の手順 3 行）は UX-7/8 の着手時にまとめて是正した
- **横展開点検（2026-08-07）**: 本セッションで実行した sweep 系 grep を洗い直した。①UX-4 の `tint` 点検（`TagChip(` / `buttonStyle(.bordered` / `accentColor`）は `head` 無しまたは全件目視で実施済み → **問題なし** ②UX-2 の周辺ピン調査は grep ではなく実測ログ → 対象外 ③本件の FAB 点検 → **該当・是正済み**。今後は「点検目的の grep に `head` を付けない」を運用ルールとする

### コードの「形」が既知バグに一致しても、**そのバグが起きている証拠にはならない**

- **症状**: ユーザー報告「好み一致のピンタップでシートが出る。そのせいでカフェ詳細に辿り着けない」。該当コードを読むと `onOpenDetail` が「`.sheet` を閉じる状態変更」と「その `.sheet` を提示している `NavigationStack` の `path` への `append`」を**同一クロージャで同期実行**していた。SwiftUI で push が握り潰される既知パターンと形が完全に一致し、しかも同じファイルの `CafeSelectionCard` は `append` を先に呼ぶ**逆順**で書かれていて、その非対称さが「片方だけ間違っている」という筋書きを補強した。**原因はこれだと判断してユーザーに報告する寸前まで行った**が、実機確認を依頼したら**ピン経由・チップ経由とも普通に遷移した**。バグは存在しなかった
- **原因の構造**: 静的解析で見つかるのは「**その症状を説明できる仮説**」であって「症状の原因」ではない。既知のアンチパターンに一致する形は、①実際に発火する条件が揃っていない ②OS バージョンで解消済み ③そもそも別経路で回避されている、のいずれでも成立してしまう。**もっともらしさが上がるほど検証を省きたくなる**のが罠で、今回は「同一ファイル内に正しい書き方が併存する」という傍証まで見つけたことが確信を過剰にした。加えて、報告文の「辿り着けない」を**機能不全**と読んだが、実際は**構成が分かりにくい**という意味だった — **ユーザーの言葉を実装用語に翻訳した時点で解釈が 1 つ確定してしまう**
- **修正パターン**: **挙動の報告には、まず挙動で答える**。再現手段が自分に無いとき（今回はシミュレータへのタップ注入ができなかった）は、**推測を原因として報告せず、ユーザーに 1 往復で確かめられる質問を投げる**。「A は動きますか / B は動きますか」のように**切り分けが進む形**にすると、答えが Yes でも No でも次の一手が決まる。今回は「両方遷移する」の一言で仮説がまとめて落ち、UX の問題だと確定できた
...
### ドメイン enum の表示変換を**ファイルローカルな private func** で持つと、書き手は常に `.name` を先に見つける

- **症状**: 記録エディタ（`CoffeeEditorView+Sections.swift`）と記録詳細（`CoffeeDetailView.swift`）で、**同じ `Form` / `LabeledContent` の並びの中で「抽出方法」だけ日本語、「精製方法」「焙煎度」は `Anaerobic` / `FullCity` と英語**だった。日本語アプリの入力画面と詳細画面の両方。しかも**エディタのファイル内には `localizedBrewMethod` が定義済み**で、3 行離れた「抽出方法」の Picker はそれを使っていた
- **原因の構造**: 同じ対応表が **18 箇所に手写し**されていた（roast 6 / brew 7 / tasting 4 / processing 1）。すべて**ファイルローカルな `private func`** で、**同じファイルを最後まで読んだ人にしか見えない**。一方 `.name` は Kotlin enum が持つプロパティなので**補完に出る**。新しい行を書く人が最初に見つけるのは必ず `.name` の方で、ヘルパの存在に気づく契機が無い。さらに `.name` は**型として正しい `String`** なので、型検査もテストも何も言わない。壊れ方が「クラッシュ」でも「空文字」でもなく「**英語の識別子がそれらしく表示される**」ため、目視レビューでも「英語表記の仕様か」と流れる。**重複が多いほど「どれが正典か」の感覚が薄れ**、19 箇所目を書く人はもうヘルパを探さない
- **修正パターン**: 変換を**型の extension に移す**（`Utilities/DomainLabels.swift`、`method.localizedLabel`）。enum 自身のメンバになるので**補完で `.name` の隣に並ぶ** — これが「ヘルパの存在に気づけない」という構造そのものの解消になる。あわせて `switch` から `default` を外し、**Kotlin 側に enum ケースが増えたらコンパイルエラー**にする（旧実装の `default: return name` は、漏れると黙って英語名を返していた）。`CategoryStat.label` のように KMP の集計結果が `String` で降ってくる経路には `static func localizedLabel(forName:)` を用意し、enum 版へ委譲して対応表は 1 本に保つ
- **教訓**: **「補完に出るか」で API の発見可能性を設計する。** 型が既に持っているプロパティ（`.name`）と競合する変換を `private func` で提供したら、書き手は構造的に `.name` を選ぶ。ドメイン型の表示変換は**最初から extension に置く**。加えて、**重複の害は「直し漏れ」より先に「使われないこと」として現れる** — 「ケースが増えたとき 18 箇所直す必要がある」は将来のリスクだが、「ヘルパがあるのに使われず英語が出ている」は**既に発現していた不具合**だった。重複を見つけたら、直し漏れの心配をする前に**まず現在の使用箇所が揃っているかを数える**
...
- **いつから壊れていたか**: 循環の両側が揃ったのは Swift 6 移行の**10 日以上前**。`.automatic` は `7904c7d`（Phase 4 / プロジェクト初期）、カメラ依存の表示数は `626fed6`（**2026-07-18** の curated ズームゲート導入）、`.automatic` のフレーミングが全国へ飛ぶようになったのは `1802db9` / `79eed0a`（**2026-07-28** の curated 東京 1 県 → 9 県 = 421 件）。**Swift 6 移行（`5cf3c50`）とは無関係で、Swift 6 前でも発生していたことをユーザーが後日確認した**
...
- **「前は起きなかった」という証言は、観測条件を確認するまで時系列の手がかりに使わない**: 「Swift 6 対応前は起きなかった」という情報を受けて移行差分（`5cf3c50`）を漁り、`assumeIsolated` に飛びついたのが 1 回目の誤修正。**後日、Swift 6 前でも発生していたことが確認された**（循環の両側は移行の 10 日以上前に成立していた）。実体は**観測機会の差**で、位置情報の許諾ダイアログは初回インストール時（またはアプリ削除後の再インストール時）にしか出ないため、TestFlight で更新を重ねる限り Background 遷移が起きず、循環は「重い」だけで殺されない。**直近の変更やビルド構成を疑う前に、「その症状を観測できる条件が揃ったのはいつか」を確認する** — 権限ダイアログ・初回起動フロー・オンボーディングが絡む症状は特に、変更時期と発現時期がずれる
...
### 画面の端に固定配置する共有オーバーレイは、適用先すべての同じ端に何があるかで位置が決まる

- **症状**: マップの検索 0 件案内をトーストで出すようにしたら、そのトーストが検索バーを覆って隠し、ユーザーが 4 秒間クエリを入力し直せない。「見つかりませんでした」と伝えるための表示が、次の操作を妨げていた
- **原因の構造**: `errorToast` は `.overlay(alignment: .top)` + `.padding(.top, 8)` で位置を**コンポーネント側に固定**している共有 modifier。適用先の `MapTabView` は上部コントロール（検索バー + フィルタチップ）を `.padding(.top, 8)` の `VStack` で置いており、**両者が同じ位置を独立に主張していた**。コンポーネントは適用先に何があるか知らず、適用先は `.errorToast(message:)` を 1 行チェーンするだけなので、どちらのコードを読んでも衝突は見えない
- **なぜ 2 ヶ月弱ぶつからなかったか**: 上部トーストは 2026-06-16 から存在したが、**出る条件がエラー時（同期失敗・検索失敗・POI ルックアップ失敗）に限られていた**ため、検索バーと同時に見える機会がほぼ無かった。そこへ「0 件案内」という**正常系で日常的に出る用途**を足した瞬間に露出した。つまり壊れたのは新機能ではなく、**既存コンポーネントの配置の前提**
- **教訓**: 共有オーバーレイに**新しい用途を足すとき、その用途が既存より高頻度・正常系なら、配置の前提を検証し直す**。「同じコンポーネントを使い回すだけ」は無変更と同義ではない。逆向きも同じで、配置を変えるときは適用先**全部**のその端を見る（今回は上部が検索欄・ナビゲーションバーで詰まっている一方、下部は FAB・シート・カードが集まる）
