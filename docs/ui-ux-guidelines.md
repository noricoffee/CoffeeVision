...
**バッジを足してもサイズは動かさない**。おすすめだけが 32pt（他の意味ピンより小さい）なのは、バッジ持ちがおすすめ 1 種だけだった時期に占有面積を相殺した名残で、**4 種すべてがバッジを持つ現在この相殺に理由はない**。ただし一律に下げると 好み一致 = 訪問済み、保存済み = おすすめ となってサイズ序列が潰れるため、現行の値を維持する。
