...
> **1.0 / 1.0.1 / 1.0.2 はすべて下の同一文言で提出している**（1.0.1・1.0.2 は書き下ろさず 1.0 の原稿をそのまま入力。2026-08-31 ユーザー確認）。したがって **`「CoffeeVision を初めてリリースしました。」` が 1.0.2 のリリースノートとしてストアに表示される** — 既知の不一致として認識した上での提出。
>
> **版ごとに書き下ろすか、この 1 本を使い回すかは未決**（tasks の 1.0.2 節に起票済み）。決めるまでこの節は「実際に提出した文言」の記録として扱い、**版を追加するときは上に積む**。

### 1.0 / 1.0.1 / 1.0.2（同一文言）
...
- **§9 What's New は 1.0 の文言をそのまま提出**（1.0.1 も同様）。結果「`CoffeeVision を初めてリリースしました。`」が 1.0.2 のノートとして表示される。§9 を「実際に提出した文言の記録」として整理し直したが、**版ごとに書き下ろすか 1 本を使い回すかは未決**
