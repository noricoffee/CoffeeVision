...
| [~] | **App Store スクリーンショットの撮影（6.9" のみ）** | 2026-08-06 着手。**6.5" は不要**と Apple 公式で確認し app-store-metadata §5 を是正（提供すれば ASC が自動縮小する）。撮影は iPhone 17 Pro Max / iOS 26.5 シミュレータ、`simctl status_bar override` で 9:41・フル電池・Wi-Fi 固定、`SIMCTL_CHILD_SEED_DUMMY_DATA=1` 付きで起動（付けないと `clearDummyData` が走り 30 件消える）。ユーザーが 6 枚撮影済みで `screenshots/6.9/` に配置。**うち `06-cafe-detail.png` は AdMob のテスト広告（`Test mode` / `developers.google.com`）が写り込み差し替え必須**。`05-record-editor.png` は上記ロケール不具合の修正後に撮り直し。§5 の 7 枚計画と実際の 6 枚は構成が違う（記録詳細・「行きたい」リストが無く分析が 2 枚）ため、確定後に §5 を追随させる |
