...
| [ ] | **索引に日付参照が 1 件も無いセクションが 5 つある**（上の作業で発見し全期間を点検した）。前文は「新規 lesson を追記したら索引にも 1 行足す」と定めているが守られていない。**07-24（1 件）/ 07-27（1 件）/ 07-31（1 件）/ 08-01（4 件）/ 08-08（7 件）= 計 14 エントリが未登録**。該当エントリに索引行を足す | 低リスク（索引への追記のみ）。主題行の振り分けは各エントリの内容で決める。**点検方法の注意**: 索引は 1 つの参照に複数エントリをまとめる場合がある（例: `(06-25、07-03 再発)`）ため、`grep -c "(MM-DD)"` の数と本文のエントリ数の差はそのまま欠落数にならない。**確実に欠落と言えるのは参照が 0 件のセクション**で、上の 5 つがそれ。差が 1〜4 ある他の日付（06-02 は 7 対 2、07-03 は 4 対 1、08-06 は 6 対 1、08-07 は 6 対 2 等）は、まとめ記載か真の欠落かをエントリ単位で確認する必要がある |
