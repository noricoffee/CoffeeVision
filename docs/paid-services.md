...
- ⚠️ **API キーの「API の制限」が Places に絞られているかは未確認**（Bundle ID 制限は確認済み）。ここが「制限なし」だと、抜かれたキーで Geocoding / Directions 等の他 Maps API を叩かれ、Places のクォータ上限を迂回される。**Cloud Console →「APIとサービス」→「認証情報」→ 該当キー →「API の制限」**で要確認
