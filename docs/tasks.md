...
| [ ] | **ASO-8** | **検索フィールドの再配分**（アプリ名 / サブタイトル / キーワード日英）。空き 25 字の充填ではなく、取れないヘッドタームを捨てて**競合の薄い複合語**へ振り替える。判断軸は ①アプリの実機能に一致 ②上位競合の評価数が一桁 ③連続した文字列として置く | 2026-08-31 起票。**需要の実測（実機の検索サジェスト）が先**（総ヒット件数はアプリ数であって検索需要ではない）。反映先は app-store-metadata §1 / §4 |
