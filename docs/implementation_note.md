...
ノート本文がスクロールしないと読めない長さになる前に、ここに **今生きてる方針だけ** を一行サマリで列挙する。陳腐化したら削除、昇格したら削除（昇格先 doc を見ればわかるため）。（最終棚卸し: 2026-07-02）
...
- ViewModel は注入 scope の Job を親にした**所有 `viewModelScope`（SupervisorJob 子スコープ）+ `clear()`** を持つ（2026-06-24。push/pop 画面の collector 増殖リーク対策）。Bridge は `deinit` で `kotlin.clear()` を呼ぶ。**KMP のコルーチン内で `runCatching` は使わない**（`CancellationException` 先行 catch + `Exception` catch のパターン）
...
- 領域: Docs
- 関連: `data-model.md` / `kmp-bridge.md` / `app-store-metadata.md` / 本ノートのサマリ

docs 全体精査（実コードとの突合を含む）で検出した重大 4 件を修正した。

1. `data-model.md`: フェーズ 10 / 12-C の変更を追随（`CoffeeRecord.tags`、`Cafe` の表示用 5 フィールド = 永続化しない旨を明記、`CoffeeStats.preferredBeanTraits` + `PreferredBeanTraits`、`CoffeeInsightProvider.summarizeBeanTraits`、SQLDelight `tags` 列 / Firestore `tags` 配列）。tasks.md 側では「data-model.md 更新」がチェック済みだったが実際は未反映だった
2. `kmp-bridge.md`: 「data-firebase は export 対象に含めない」を実体（`shared/framework/build.gradle.kts` は export 済み）に合わせて修正。architecture.md と整合
3. `app-store-metadata.md`: CoffeeRecord 主体へ全面改訂（詳細は同ファイル変更履歴 2026-07-02）
4. 本ノート「現在生きてる方針サマリ」を現状へ全面更新（旧サマリは 2026-06-15 頃で凍結し Visit 系記述が残っていた）
