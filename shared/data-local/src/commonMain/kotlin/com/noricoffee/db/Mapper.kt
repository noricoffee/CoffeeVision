...
internal fun CoffeeRecord.toRow(): Coffee_record = Coffee_record(
    ...
    cafe_photo_attributions = cafe?.photoAttributions?.encodeToJson(),
...
internal fun Coffee_record.toDomain(photos: List<DomainPhoto>): CoffeeRecord {
    val cafe = if (cafe_place_id != null && cafe_name != null) {
        Cafe(
            ...
            photoAttributions = cafe_photo_attributions?.decodeStringList() ?: emptyList(),
    ...
    return CoffeeRecord(
        ...
        photos = photos,
        name = name,
        brewMethod = BrewMethod.entries.firstOrNull { it.name == brew_method } ?: BrewMethod.Other,
        origin = origin,
        region = region,
        variety = variety,
        processing = processing?.let { name -> ProcessingMethod.entries.firstOrNull { it.name == name } },
        roastLevel = roast_level?.let { name -> RoastLevel.entries.firstOrNull { it.name == name } },
        cup = cup,
