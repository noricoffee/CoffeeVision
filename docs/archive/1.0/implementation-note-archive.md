# 実装ノート アーカイブ（2026-06〜2026-08 / 1.0 まで）

[`implementation_note.md`](../../implementation_note.md) から切り出した**凍結済み**の過去分（1.0 リリースまで）。月ごとにセクションを分けて格納する。次のリリース分は `docs/archive/1.1/` のように新しいフォルダを切る。

| 月 | 件数 | 内容 | 移送日 |
|---|---|---|---|
| 2026-06 | 36 件 | フェーズ 1〜14 相当の完了記録 | 2026-07-25 |
| 2026-07 | 73 件 | フェーズ 15〜19 / 広告 / 分析 / docs 棚卸し第 1〜2 巡 | 2026-08-11 |
| 2026-08 | 30 件 | Swift 6 移行 / Swift コードレビュー是正 / UI-UX レビュー是正 / App Store 提出（1.0） | 2026-08-21 |

- **ここには追記しない。** 新しいエントリは常に `implementation_note.md` へ書く（追記が止まった月をまとめて移送するだけの受け皿）
- 他 doc やコードコメントの「implementation_note 2026-0X-XX エントリ」という参照は**この doc を指す**（参照は日付で引く運用で、行き先はこの 1 ファイルに固定されているため、参照側の書き換えは不要）
- 現在生きている方針のサマリは `implementation_note.md` の「現在生きてる方針サマリ」が正本。ここの記述は**当時の判断の記録**であり、現行仕様とは限らない
- **本 doc に行数閾値は適用しない。** `curate-doc` skill のフロー型閾値（1200 行）は「追記が止まった月をアーカイブへ送れ」という発火条件であり、既にその移送先である本 doc には意味を持たない。凍結ドキュメントなので通読されず、日付での grep で引かれる
- 分離の判断は `implementation_note.md` 2026-07-25 エントリ（2026-06 分）/ 2026-08-11 エントリ（2026-07 分）
...
**「数え上げを書かない」の適用範囲がサマリ限定ではなかった。** 2026-08-01 に本ノートのサマリへ課した規約だが、今回コード KDoc で**同じ失敗が独立に発生していた**ことが分かった。件数は依存が 1 つ増えるたびに全項目がまとめて嘘になり、増やした本人はソースしか見ないので気づけない — この性質は doc かコメントかに依存しない。横断点検を `shared/ iosApp/` × `*.kt` `*.swift` × `docs/**` × **`.claude/agent-memory/**`** まで広げ、最終的に **13 箇所**を是正した（コード 5 / doc 6 / エージェントメモリ 2）。

**新しく分かったこと 2 つ**:

- **エージェントの永続メモリが sweep 対象から漏れていた。** `.claude/agent-memory/**` は git 管理下で doc と同じ速度で陳腐化するのに、これまで一度も横断点検の範囲に入っていなかった。実際 `ios-engineer` のメモリには「`Cafe` は Swift 側で **14 引数**の init 1 本」（現在 15）、`kmp-engineer` のメモリには `AppContainer` の「**7 引数版 iOS / 6 引数版 Android**」（現在 8 / 7）と、**どちらも実数として壊れた記述**が残っていた。しかもこれは doc より害が大きい — エージェントは着手時にメモリを読んで**それを前提にコードを書く**ので、誤った数字が次の実装に直接入り込む。今後の sweep は `.claude/agent-memory/**` を必ず含める
- **「N と書くな」は線引きとして粗すぎた。** sweep は 40 件超をヒットさせたが是正が要ったのは一部で、残りは `TastingScores` の「5 要素」や `PreferenceMatchAxis` の「4 軸」のような**閉じた集合**だった。これらは増やせば全構築箇所がコンパイルエラーになるので静かには壊れない。壊れるのは `Cafe` のプロパティや `AppContainer` の引数のように**デフォルト値付きで末尾に足せてコンパイラが何も言わない集合**。正しい規則は「**コンパイラが増加を検出しない集合を数えるな**」で、この判別ができると sweep のノイズが一気に減る（この線引きは lessons と両エージェントのメモリに渡した）

`data-model.md` 自身も同じ地雷を 6 個持っていた（「9 フィールド」の再掲）。§1.2 のフィールド名列挙を唯一の正本と明記し、他は参照表現に置換した。
...
**API 制限の Places 限定も同日に完了し、キー悪用対策は 3 点そろった。** クォータ上限だけでは不十分で、キーが他 Maps API（Geocoding / Directions 等）を叩けると**Places に付けた天井を素通りされる**。逆に API 制限だけでも Places 自体を叩かれ放題なので、**予算アラート（気づく）/ クォータ上限（Places の量に天井）/ API 制限（迂回路を塞ぐ）は 3 つで 1 つの防御**として扱う。副作用として、新しい Google API を使い始めるときはキーの許可リストへの追加が必要になる（忘れると `403` で機能が動かない）。

2026-07-08 に書いた方針が実施されるまで **1 か月以上、しかも提出後まで**かかった。方針を書いた場所（本ノートのサマリ）と、実施したかを追跡する場所（提出前チェックリスト）が分かれていて繋がっていなかったのが原因なので、以後は**本番環境への設定を伴う方針を書いたら、その場でチェックリスト側にも行を立てる**。
