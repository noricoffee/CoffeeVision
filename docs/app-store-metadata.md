...
**撮影済み 6 枚**（2026-08-06 / ユーザー確定）。原本は `screenshots/6.9/`、ASC アップロード順 = ファイル名の番号順 = ストアでの表示順。

| # | ファイル | 画面 | 訴求ポイント | キャプション案 |
|---|---------|------|------------|--------------|
| 1 | `01-map.png` | マップ画面 | 訪問済み / 好み一致 / 保存済みのピン + 埋め込み検索バー | 「行ったお店も、これから行くお店も。」 |
| 2 | `02-analyze-summary.png` | 分析画面（サマリ） | 総杯数・平均評価 + 評価分布 + テイスティングレーダー | 「記録が増えるほど、好みが見えてくる。」 |
| 3 | `03-analyze-suggest.png` | 分析画面（傾向と提案） | 4 軸の好み傾向 + 好みの豆の言語化 + 未体験豆の提案 | 「次に試すべき一杯が、見えてくる。」 |
| 4 | `04-record-list.png` | コーヒー記録一覧画面 | 月別セクションの振り返り + 検索 + FAB | 「コーヒー巡りの軌跡が、ここに。」 |
| 5 | `05-record-editor.png` | コーヒー記録 作成 / 編集画面 | テイスティング 5 軸のスライダー + 星評価 | 「抽出も焙煎も、こだわりを残せる。」 |
| 6 | `06-cafe-detail.png` | カフェ詳細画面 | 店舗写真・評価・営業状況 + 同じ店の記録の集約 | 「お気に入りのお店を、何度でも。」 |
...
`simctl` のスクリーンショットは全ピクセル不透明でも**アルファチャンネルを持つ**ため、ASC の「alpha channels or transparencies を含む画像は不可」に抵触しうる。`screenshots/flatten-alpha.swift` で白背景に合成した PNG を書き出す:

```
xcrun swift screenshots/flatten-alpha.swift screenshots/6.9 screenshots/submit
```

出力先 `screenshots/submit/` は **git 非追跡**（原本からいつでも再生成できるため）。**ASC にはこちらをアップロードする**。
