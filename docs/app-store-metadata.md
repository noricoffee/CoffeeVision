...
> ⚠️ **`04-record-list.png` は再撮影が必要**（2026-08-07 時点で未実施）。撮影時点では右下に FAB があったが、UX-3 で追加ボタンをナビバー右上へ移したため**現物と一致しない**。`screenshots/6.9/` の原本と `screenshots/submit/` の焼き込み版の両方を撮り直すこと。他の 5 枚は UI 変更の影響を受けていないが、UX-1（おすすめピンに★バッジ追加）が `01-map.png` に写り込む可能性があるため、提出前に目視で突き合わせる。
