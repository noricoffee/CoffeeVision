...
- **`docs/**.md` が 500 行を超えたら棚卸しする**（PostToolUse フック `check-file-size.sh` が警告）。手順は `curate-doc` skill: **陳腐化チェック（コードと突き合わせ）→ 縮約 → 分離**の 3 段で、**縮約から入らない**（行数だけ見て整形すると古い記述が綺麗になるだけで、実装との乖離が残る。data-model.md 1246 行の棚卸しでは陳腐化 6 件 + 欠落 2 件 + データ欠損バグ 1 件が出た。2026-07-25）
