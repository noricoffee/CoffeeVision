...
ピンの視覚的な強弱は **サイズと色（彩度）だけで表す**。輪郭・影は全ピン共通の「地図から切り離すための処理」であり、強調の手段に使わない。**バッジはこの規則の対象外** — 強弱ではなく*意味*を載せる枠で、現在は訪問回数（訪問済み）と★（おすすめ）の 2 種。
...
| バッジ | 直径 18pt の**白地の円 + 自色のシンボル / 文字**、`.offset(x: 4, y: -4)` で右上に重ねる。訪問済み = 訪問回数（2 回以上のみ。10 以上は `9+`）、おすすめ = `star.fill` |
