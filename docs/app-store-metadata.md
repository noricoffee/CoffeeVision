...
>
> **パッチ版は「軽微な〜」の 1 行**という運用が 1.0.1 / 1.0.2 で定着している（文言は毎回微妙に違うが、揃える必要は無いと判断）。機能追加を含む版では書き下ろすこと。
>
...
### 1.0.2（2026-08-31 提出 / 審査待ち）

```
軽微な修正を行いました
```

> 提出直後で未配信のため**実測できていない**（ユーザー申告）。配信後に上のコマンドで照合すること。
