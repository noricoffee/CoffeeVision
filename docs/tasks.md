...
| [~] | プライバシーポリシー / サポートページの公開（記録データをサービス改善に使用する旨の明記） | 本文は 2026-07-21 起草、**2026-08-06 に GitHub Pages で公開**（`.github/workflows/pages.yml` / `docs/legal/` のみを配信し設計 doc は Web 公開しない）。名義 = `noricoffee`、2 ページを相対パスで相互リンク、`index.html` 新設でルート 404 回避。アプリ内の `https://example.com/privacy`（404）も実 URL へ差し替え（`DataConsentOnboardingView`、親がフラグ無し `** BUILD SUCCEEDED **` 再確認）。副産物: **起草者向けの指示文が本文に残っていた**（§5 の `.note` div「※ App Store の年齢レーティング設定と整合させてください」）ため削除 — 公開すればユーザーに見える状態だった。**残 1 点（ユーザー作業）**: サポート用メールアドレスの発行 → 2 ページの `[サポートメールアドレス]` に反映。9-6 協調フィルタ（未実装）は実装着手時に追記。URL は app-store-metadata §6.4 |
