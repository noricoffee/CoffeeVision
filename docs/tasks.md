...
> 実装ノートが 2160 行 / 約 120 エントリに肥大化したため、ノート自身の運用ルール（陳腐化は削除可 / 昇格時は削除）に沿って棚卸し。陳腐化エントリは削除（git 履歴がアーカイブ、ユーザー承認済み）、シリーズは最終形へ統合、冗長な事前設計は確定判断のみに圧縮。

| 状態 | タスク | 備考 |
|------|------|------|
| [x] | 陳腐化エントリ（旧 Visit / sharedLogic 時代・後続で置換済みの暫定対応 約 40 件）を削除 | 2026-07-04 完了 |
| [x] | シリーズエントリ（B-1×5 / B-4×3 / アカウント×4 / Q&A×3 / 12-A×3 / テイスティング×2 / Places 疎通×2 ほか）を最終形 1 本へ統合 | 2026-07-04 完了 |
| [x] | 現役エントリの圧縮 + 見出しスタイル統一（`###` / 日付順）+ サマリ棚卸し日付更新 + 棚卸し記録エントリ追記 | 2026-07-04 完了。2160 行 / 297KB → 691 行 / 79KB（約 120 → 64 エントリ） |
| [x] | 検証: 行数・見出し形式・外部参照（coding-conventions → 06-24 / requirements → 06-19 分析・06-22 B-4・Future Direction）の生存確認 | 2026-07-04 完了。参照 4 系統すべて生存、見出しは `###` + 日付昇順に統一 |
