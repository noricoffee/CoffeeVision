...
**残課題:**
- 各モジュールの KDoc コメント中に旧 `sharedLogic/androidMain` 等の経緯記述が残る（`AppContainer.kt` / `AuthRepository.kt` / `RemoteVisitDataSource.kt` / `kmp.library.gradle.kts` 計 5 箇所）。機能影響なし。Phase 3 着手前後で気付いたタイミングで整理
